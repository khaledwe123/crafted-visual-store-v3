...
// Public checkout payment selection endpoint.
// IMPORTANT: this does NOT mark an order as paid. It only records the selected
// payment method and keeps the order awaiting verified payment/manual review.
app.post('/api/orders/:orderNo/payment-method', writeLimiter, (req,res)=>{
  const orderNo = String(req.params.orderNo || '').trim();
  const { payment_method, payment_provider, expected_total } = req.body || {};
  const allowedMethods = ['Geidea','Mada','Visa','Mastercard','Apple Pay','Bank Transfer','Cash on Delivery','Pay in Showroom'];
  const method = String(payment_method || '').trim();
  if(!allowedMethods.includes(method)) return res.status(400).json({error:'Unsupported payment method'});

  const o = db.prepare('SELECT * FROM orders WHERE order_no=?').get(orderNo);
  if(!o) return res.status(404).json({error:'Order not found'});

  if(expected_total != null){
    const expected = Number(expected_total);
    const actual = Number(o.total_amount || 0);
    if(Number.isFinite(expected) && Math.abs(expected - actual) > 1){
      return res.status(409).json({error:'Order total mismatch. Please review your cart again.'});
    }
  }

  const provider = payment_provider || (['Mada','Visa','Mastercard','Apple Pay','Geidea'].includes(method) ? 'Geidea' : 'Manual');
  const nextPaymentStatus = provider === 'Geidea' ? 'awaiting_gateway_payment' : 'awaiting_manual_payment';
  const nextOrderStatus = provider === 'Geidea' ? 'awaiting_payment' : 'payment_pending_review';

  db.prepare('UPDATE orders SET payment_method=?, payment_status=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?')
    .run(method, nextPaymentStatus, nextOrderStatus, o.id);

  const c = json(o.customer_json,{});
  const body = `Order ${o.order_no} selected ${method}. Payment status: ${nextPaymentStatus}. Total: SAR ${Math.round(Number(o.total_amount||0))}.`;
  db.prepare('INSERT INTO crm_activities(customer_id,order_id,type,channel,subject,body,status) VALUES(?,?,?,?,?,?,?)')
    .run(o.customer_id,o.id,'payment_method_selected','system','Payment method selected '+o.order_no,body,'done');

  if(c.email) db.prepare('INSERT INTO automation_outbox(order_id,customer_id,channel,recipient,subject,body) VALUES(?,?,?,?,?,?)')
    .run(o.id,o.customer_id,'email',c.email,'Payment pending for '+o.order_no,body);
  if(c.mobile) db.prepare('INSERT INTO automation_outbox(order_id,customer_id,channel,recipient,body) VALUES(?,?,?,?,?)')
    .run(o.id,o.customer_id,'whatsapp',c.mobile,body);
  ...
  res.json({
    ...
    geidea_configured:false,
    message: provider === 'Geidea'
      ? 'Geidea API is not configured yet. Order is saved and waiting for gateway integration.'
      : 'Order is saved and waiting for manual payment verification.'
  });
});
