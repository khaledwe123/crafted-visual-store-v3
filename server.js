...
app.get('/api/version',(req,res)=>res.json({version:'CRAFTED-VISUAL-SUPERADMIN-AUTHORITY-PATCH-20260609-20', superadminFix:true, mediaLibrary:true, publicTopRibbon:false, publicAnalyticsMenu:false, adminLogin:true, time:new Date().toISOString()}));
...
app.post('/api/admin/login', authLimiter, (req,res)=>{
  ...
  res.json({
    ...
    user:{id:user.id,name:user.name,email:user.email,role:(role === 'superadmin' || isDefaultSuperAdminEmail(user.email)) ? 'superadmin' : user.role,isSuperAdmin:(role === 'superadmin' || isDefaultSuperAdminEmail(user.email)),permissions}
  });
});
