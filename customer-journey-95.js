...
/* FRONTEND SHOP CARD ACTION + RED DISCOUNT FIX - 20260609
   Replaces inline onclick shop cards because production CSP blocks inline handlers. */
(function(){
  function esc(v){ return String(v == null ? '' : v).replace(/[&<>"]/g, function(m){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[m]; }); }
  function txt(en, ar){ return document.documentElement.dir === 'rtl' ? (ar || en) : en; }
  function productName(p){ return (window.displayName ? displayName(p) : (p.name || p.title || 'Product')); }
  function productCat(p){ return (window.displayCategory ? displayCategory(p) : (p.category || '')); }
  function imgFor(p){
    try{ if(window.firstImage) return firstImage(p); }catch(e){}
  ...
  function priceBlock(p){
    p = window.normalizeProduct ? normalizeProduct(p) : p;
    const low = window.cvLowestVariant ? cvLowestVariant(p) : {before:Number(p.price||0), final:(window.finalPrice?finalPrice(p):Number(p.price||0)), discount:Number(p.discountPercent||0)};
    if(low.discount > 0){
      return '<div class="cj-starting discount-visible">' +
        '<small>' + txt('Starting from','يبدأ من') + '</small><br>' +
        '<span class="old-price" style="text-decoration:line-through;opacity:.65;">' + money(low.before) + '</span><br>' +
        '<strong class="discount-price" style="color:#c62828!important;font-weight:900;">' + money(low.final) + '</strong> ' +
        '<span class="discount-badge" style="background:#c62828!important;color:#fff!important;">' + low.discount + '% OFF</span>' +
      '</div>';
    }
    return '<div class="cj-starting">' + txt('Starting from','يبدأ من') + ' <strong>' + money(low.final) + '</strong></div>';
  }
  function fixedRenderProducts(list){
    const grid = document.getElementById('productGrid');
    if(!grid || !Array.isArray(list)) return;
    if(!list.length){
      grid.innerHTML = '<div class="empty-products"><h3>' + txt('No products showing yet','لا توجد منتجات حالياً') + '</h3><p>' + txt('No products are currently published.','لا توجد منتجات منشورة حالياً.') + '</p></div>';
      return;
    }
    grid.innerHTML = list.map(function(raw){
      const p = window.normalizeProduct ? normalizeProduct(raw) : raw;
      const pid = String(p.id || p._dbId || '');
      const colors = Object.entries(p.colors || {}).slice(0,5).map(function(entry){ const v=entry[1]||{}; return '<span title="'+esc(entry[0])+'" style="background:'+esc(v.hex || '#ccc')+'"></span>'; }).join('');
      const fabrics = (p.fabricOptions || []).slice(0,3).map(function(f){ return '<small>'+esc(f.label || f)+'</small>'; }).join('');
      return '<div class="card cj-product-card product-card" data-product-id="'+esc(pid)+'">' +
        '<button class="cj-wish wishlist-heart" type="button" data-shop-action="wishlist" data-product-id="'+esc(pid)+'" title="Wishlist">♥</button>' +
        '<img src="'+esc(imgFor(p))+'" alt="'+esc(productName(p))+'">' +
        '<div class="card-body">' +
          '<h3>'+esc(productName(p))+'</h3>' +
          '<p>'+esc(productCat(p))+'</p>' +
          (window.starDisplay ? starDisplay(pid) : '') +
          priceBlock(p) +
          '<div class="mini-swatches">'+colors+'</div>' +
          '<div class="cj-fabric-tags">'+fabrics+'</div>' +
          '<div class="cj-delivery">'+txt('Made to order: 15–20 working days','تفصيل حسب الطلب: 15–20 يوم عمل')+'</div>' +
          '<div class="cj-card-actions">' +
            '<button class="btn secondary" type="button" data-shop-action="quick" data-product-id="'+esc(pid)+'">'+txt('Quick View','عرض سريع')+'</button>' +
            '<button class="btn primary" type="button" data-shop-action="customize" data-product-id="'+esc(pid)+'">'+txt('Customize','خصص المنتج')+'</button>' +
          '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  }
  window.renderProducts = fixedRenderProducts;
  try{ renderProducts = fixedRenderProducts; }catch(e){}
  document.addEventListener('click', function(e){
    ...
    if(action === 'quick' || action === 'customize'){
      ...
      if(action === 'customize' && typeof window.openCustomizeProduct === 'function') window.openCustomizeProduct(id);
      else if(typeof window.openProduct === 'function') window.openProduct(id);
    }
  }, true);
  setTimeout(function(){ try{ if(window.products && window.products.length) fixedRenderProducts(window.products); else if(typeof applySortAndFilter === 'function') applySortAndFilter(); }catch(e){} }, 500);
})();
