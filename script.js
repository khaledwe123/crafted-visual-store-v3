...
(function(){
  ...
  async function loadDiscountCodes(){
    var out = [];
    try{
      var res = await fetch('/api/discounts', {cache:'no-store'});
      if(res.ok){
        var rows = await res.json();
        if(Array.isArray(rows)) out = out.concat(rows.map(normalizeDiscountCode));
      }
    }catch(e){}

    ['discountCodes','checkoutDiscountCodes','cvDiscountCodes'].forEach(function(key){
      try{
    ...
    var map = new Map();
    out.filter(function(c){ return c.code && c.percent > 0; }).forEach(function(c){ map.set(c.code, c); });
  ...
  async function applyCartDiscountCode(){
    ...
    var codes = await loadDiscountCodes();
    var found = codes.find(function(c){ return c.code === codeText && c.active !== false; });
    if(!found){
      appliedCartDiscount = null;
      saveCartDiscount();
