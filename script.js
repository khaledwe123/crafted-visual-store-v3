...
/* === CV COLOR TAB IMAGE SWITCH ONLY FIX - 20260610 ===
   Scope: frontend product modal only.
   Fixes color tabs and thumbnails without changing backend/admin/discount logic.
*/
(function(){
  'use strict';
  ...
  function cvImagesForColor(product, colorName){
    product = product || window.currentProduct || currentProduct || {};
    const colors = product.colors || {};
    const set = colors[colorName] || colors[String(colorName || '').trim()] || null;
    if(set){
      const out = [];
      if(Array.isArray(set.imageMeta)) set.imageMeta.forEach(m => { if(m && m.url) out.push(cvImg(m.url)); });
      if(Array.isArray(set.images)) set.images.forEach(url => { if(url) out.push(cvImg(url)); });
  ...
  window.getColorImages = getColorImages = function(color){
    return cvImagesForColor(window.currentProduct || currentProduct, color);
  };
  ...
  window.renderThumbs = renderThumbs = function(){
    ...
    if(!box || !currentProduct) return;
    const imgs = cvImagesForColor(currentProduct, selectedColor);
    box.innerHTML = imgs.map(function(img){
      return '<img src="' + cvEsc(img) + '" data-thumb-img="' + cvEsc(img) + '" class="' + (img === selectedImage ? 'active' : '') + '" alt="">';
    }).join('');
  };
  ...
  window.updateModalImage = updateModalImage = function(){
    ...
    if(!img) return;
    const fallback = cvImagesForColor(currentProduct, selectedColor)[0] || 'assets/products/product_01.png';
    img.src = selectedImage || fallback;
  };
  ...
  document.addEventListener('click', function(e){
    const colorBtn = e.target.closest('#colorOptions .color-chip[data-color-name]');
    if(colorBtn){
      e.preventDefault();
      e.stopPropagation();
    ...
    const thumb = e.target.closest('#thumbs img[data-thumb-img]');
    if(thumb){
      e.preventDefault();
      e.stopPropagation();
      selectImage(thumb.getAttribute('data-thumb-img'));
    }
  }, true);
})();
