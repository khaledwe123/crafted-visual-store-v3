...
function apiProductToFrontend(row){
  ...
  if((!data || !Object.keys(data).length) && row && row.data_json){
    try{ data = typeof row.data_json === 'string' ? JSON.parse(row.data_json) : row.data_json; }catch(e){ data = {}; }
  }
  const product = Object.assign({}, data || {});
  product._dbId = row.id || product._dbId;
  product.id = product.id || row.sku || String(row.id);
  product.name = product.name || row.name_en || '';
  product.name_ar = product.name_ar || row.name_ar || '';
  product.category = product.category || row.category_name || row.category || 'Beds';
  product.category_ar = product.category_ar || row.category_ar || product.category || '';
  product.description = product.description || row.description_en || '';
  product.description_ar = product.description_ar || row.description_ar || '';
  product.price = Number(product.price || row.base_price || 0);
  product.vatRate = Number(product.vatRate || row.vat_rate || settings.vat_rate || 15);
  product.active = row.active === 0 ? false : product.active !== false;
  return product;
}

function prototypeProductsReadFront(){
  // Shop fallback must read the same local keys used by the admin product save flow.
  // Keep this frontend-only: no backend/admin logic is changed.
  const raw =
    localStorage.getItem('cvPrototypeProducts') ||
    localStorage.getItem('adminProducts') ||
    localStorage.getItem('cms_products') ||
    localStorage.getItem('products') ||
    sessionStorage.getItem('cvPrototypeProducts') ||
    sessionStorage.getItem('adminProducts') ||
    sessionStorage.getItem('cms_products') ||
...
function applySortAndFilter(){
  ...
  if(activeCategory !== "All"){
    const wanted = String(activeCategory || '').trim().toLowerCase();
    list = list.filter(p =>
      String(p.category || '').trim().toLowerCase() === wanted ||
      String(p.category_ar || '').trim().toLowerCase() === wanted
    );
  }
...
/* === CV SHOP VIEW ONLY FINAL FIX ===
   Purpose: make /shop.html display products saved from backend without changing admin/backend.
   Fixes robust parsing of API rows and forces one safe shop refresh after page load.
*/
(function(){
  function cvParseData(value){
    if(!value) return {};
    if(typeof value === 'object') return value;
    try{ return JSON.parse(value); }catch(e){ return {}; }
  }

  function cvSafeNumber(v, fallback){
    const n = Number(v);
    return Number.isFinite(n) ? n : (fallback || 0);
  }

  function cvApiProductRowToShop(row){
    row = row || {};
    const data = Object.assign({}, cvParseData(row.data_json), cvParseData(row.data));
    const product = Object.assign({}, data);

    product._dbId = row.id || product._dbId;
    product.id = product.id || row.sku || String(row.id || Date.now());
    product.name = product.name || row.name_en || row.name || '';
    product.name_ar = product.name_ar || row.name_ar || '';
    product.category = product.category || row.category_name || row.category || 'All Products';
    product.category_ar = product.category_ar || row.category_ar || product.category;
    product.description = product.description || row.description_en || '';
    product.description_ar = product.description_ar || row.description_ar || '';
    product.price = cvSafeNumber(product.price || row.base_price, 0);
    product.vatRate = cvSafeNumber(product.vatRate || row.vat_rate || (window.settings && settings.vat_rate), 15);
    product.active = row.active === 0 || row.active === false || product.active === false ? false : true;

    if(!product.colors || !Object.keys(product.colors || {}).length){
      const img = product.image || (Array.isArray(product.gallery) ? product.gallery[0] : '');
      product.colors = { Standard: { hex:'#cccccc', code:'', images: img ? [img] : [] } };
    }
    return typeof normalizeProduct === 'function' ? normalizeProduct(product) : product;
  }

  window.apiProductToFrontend = apiProductToFrontend = cvApiProductRowToShop;

  async function cvFetchShopProducts(){
    try{
      const res = await fetch('/api/products', { cache:'no-store' });
      if(res.ok){
        const rows = await res.json();
        if(Array.isArray(rows) && rows.length) return rows.map(cvApiProductRowToShop).filter(p => p && p.active !== false);
      }
    }catch(e){ console.warn('Shop product API unavailable.', e); }

    const keys = ['cvPrototypeProducts','adminProducts','cms_products','products'];
    for(const storage of [localStorage, sessionStorage]){
      for(const key of keys){
        try{
          ...
          if(Array.isArray(list) && list.length) return list.map(p => typeof normalizeProduct === 'function' ? normalizeProduct(p) : p);
        }catch(e){}
      }
    }
    return [];
  ...
  async function cvRefreshShopViewOnly(){
    const grid = document.getElementById('productGrid');
    if(!grid) return;
    const fresh = await cvFetchShopProducts();
    if(fresh.length){
      products = fresh;
      ...
      else if(typeof renderProducts === 'function') renderProducts(products);
    }
  }

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(cvRefreshShopViewOnly, 400); });
  }else{
    setTimeout(cvRefreshShopViewOnly, 400);
  }
})();

/* === CV SHOP VISIBILITY ROOT FIX 20260610 ===
   Scope: frontend shop view only.
   Purpose:
   1) customer-journey-95.js calls money(...) as a global helper; expose it safely.
   2) Some shop modules request /api/products/shop, while the backend exposes /api/products.
      Bridge only that frontend request without touching backend/admin.
   3) Re-fetch published products and render them after all shop scripts load.
*/
(function(){
  'use strict';
  if(window.__cvShopVisibilityRootFix20260610) return;
  window.__cvShopVisibilityRootFix20260610 = true;

  window.money = window.money || function(v){
    var n = Number(v || 0);
    return 'SAR ' + Math.round(Number.isFinite(n) ? n : 0).toLocaleString();
  };
  ...
  function isVisibleProduct(row, product){
    if(!row && !product) return false;
    var active = row && row.active;
    var status = String((product && (product.status || product.publishStatus || product.visibility)) || (row && (row.status || row.visibility)) || '').toLowerCase();
    var published = product && product.published;
    var deleted = product && (product.deleted || product.isDeleted || product.softDeleted);

    if(deleted) return false;
    if(active === 0 || active === false || active === '0' || active === 'false') return false;
    if(status && ['inactive','disabled','draft','hidden','unpublished','deleted','archived'].includes(status)) return false;
  ...
  function rowToShopProduct(row){
    row = row || {};
    var data = Object.assign({}, parseJSON(row.data_json), parseJSON(row.data));
    var p = Object.assign({}, data);

    p._dbId = row.id || p._dbId;
    p.id = p.id || row.sku || String(row.id || Date.now());
    p.name = p.name || row.name_en || row.name || '';
    p.name_ar = p.name_ar || row.name_ar || '';
    p.category = p.category || row.category_name || row.category || 'All Products';
    p.category_ar = p.category_ar || row.category_ar || p.category;
    ...
    p.vatRate = Number(p.vatRate || row.vat_rate || (window.settings && window.settings.vat_rate) || 15);
    p.discountPercent = Number(p.discountPercent || p.discount || row.discount_percent || 0);
    p.active = true;

    if(!p.colors || !Object.keys(p.colors || {}).length){
      var img = p.image || (Array.isArray(p.gallery) && p.gallery[0]) || '';
      p.colors = { Standard: { hex:'#cccccc', code:'', images: img ? [img] : [] } };
    }

    return (typeof window.normalizeProduct === 'function') ? window.normalizeProduct(p) : p;
  }

  window.apiProductToFrontend = function(row){
    return rowToShopProduct(row);
  };

  if(!window.__cvProductsShopFetchBridge){
    window.__cvProductsShopFetchBridge = true;
    var originalFetch = window.fetch ? window.fetch.bind(window) : null;
    if(originalFetch){
      window.fetch = function(input, init){
        var url = (typeof input === 'string') ? input : (input && input.url) || '';
        if(/\/api\/products\/shop(?:\?|$)/.test(url)){
          input = url.replace('/api/products/shop', '/api/products');
        }
      ...
      try{
        ...
        if(Array.isArray(rows) && rows.length){
          return rows.map(function(row){
            var data = Object.assign({}, parseJSON(row.data_json), parseJSON(row.data));
            return {row:row, data:data, product:rowToShopProduct(row)};
          }).filter(function(x){ return isVisibleProduct(x.row, x.data) && x.product && x.product.name; })
  ...
  async function forceShopRefresh(){
    var grid = document.getElementById('productGrid');
    if(!grid) return;
    var fresh = await fetchPublishedProducts();
    if(!fresh.length) return;
    window.products = products = fresh;
    if(typeof renderDynamicCategories === 'function') renderDynamicCategories();
    if(typeof applySortAndFilter === 'function') applySortAndFilter();
    else if(typeof renderProducts === 'function') renderProducts(fresh);
  }

  window.addEventListener('load', function(){ setTimeout(forceShopRefresh, 300); setTimeout(forceShopRefresh, 1200); });
})();

/* CRAFTED-VISUAL-MODAL-PICTURE-CLOSE-FIX-20260610
   Focus only: product modal close + product picture thumbnails.
   Reason: production CSP blocks inline onclick handlers, so we bind safe JS listeners.
*/
(function(){
  'use strict';
  if(window.__cvModalPictureCloseFix20260610) return;
  window.__cvModalPictureCloseFix20260610 = true;
  ...
  window.closeModal = function(){
    ...
    if(box) box.remove();
    var badge = document.getElementById('customizeModeBadge');
    if(badge) badge.remove();
  };

  window.selectImage = function(img){
    selectedImage = img || selectedImage || '';
    var main = document.getElementById('modalImage');
    if(main && selectedImage) main.src = selectedImage;
    if(typeof renderThumbs === 'function') renderThumbs();
  };

  window.renderThumbs = renderThumbs = function(){
    var thumbs = document.getElementById('thumbs');
    if(!thumbs || !currentProduct) return;
    var imgs = [];
    try{
      imgs = typeof getColorImages === 'function' ? getColorImages(selectedColor) : [];
    }catch(e){ imgs = []; }
    if(!Array.isArray(imgs) || !imgs.length){
      try{ imgs = [typeof firstImage === 'function' ? firstImage(currentProduct) : ''].filter(Boolean); }catch(e){ imgs = []; }
    }
    thumbs.innerHTML = imgs.map(function(img){
      return '<img src="' + esc(img) + '" data-modal-thumb="' + esc(img) + '" class="' + (img === selectedImage ? 'active' : '') + '" alt="Product image">';
    }).join('');
  };

  document.addEventListener('click', function(e){
    var closeBtn = e.target && e.target.closest && e.target.closest('#productModal .modal-close, #productModal .close, [data-close-modal]');
    if(closeBtn){
      e.preventDefault();
      e.stopPropagation();
      window.closeModal();
    ...
    var thumb = e.target && e.target.closest && e.target.closest('#thumbs [data-modal-thumb]');
    if(thumb){
      e.preventDefault();
      e.stopPropagation();
      window.selectImage(thumb.getAttribute('data-modal-thumb'));
    }
  }, true);
...
/* === CV COLOR TAB IMAGE SWITCH ONLY FIX - 20260610 SAFE ===
   Scope: frontend product modal only.
   Fixes color tabs/images under CSP without touching shop loading, admin, backend, or discounts.
*/
(function(){
  'use strict';
  if(window.__cvColorTabImageSwitchOnlyFixSafe) return;
  window.__cvColorTabImageSwitchOnlyFixSafe = true;
  ...
  function cvImg(url){
    if(typeof cvImgUrl === 'function') return cvImgUrl(url);
    if(!url) return 'assets/products/product_01.png';
    return String(url);
  }
  function cvColorKey(product, colorName){
    var colors = (product && product.colors) || {};
    if(colors[colorName]) return colorName;
    var wanted = String(colorName || '').trim().toLowerCase();
    return Object.keys(colors).find(function(k){ return String(k || '').trim().toLowerCase() === wanted; }) || colorName;
  }
  function cvImagesForColorOnly(product, colorName){
    product = product || currentProduct || {};
    var colors = product.colors || {};
    var key = cvColorKey(product, colorName);
    var set = colors[key] || null;
    if(set){
      var out = [];
      if(Array.isArray(set.imageMeta)){
        set.imageMeta.forEach(function(m){ if(m && m.url) out.push(cvImg(m.url)); });
      }
      if(Array.isArray(set.images)){
        set.images.forEach(function(url){ if(url) out.push(cvImg(url)); });
      }
      if(set.image) out.push(cvImg(set.image));
      var unique = Array.from(new Set(out.filter(Boolean)));
      if(unique.length) return unique;
    }
    if(Array.isArray(product.gallery) && product.gallery.length) return product.gallery.map(cvImg).filter(Boolean);
    if(typeof firstImage === 'function') return [firstImage(product)].filter(Boolean);
    return ['assets/products/product_01.png'];
  }

  window.getColorImages = getColorImages = function(color){
    return cvImagesForColorOnly(currentProduct, color);
  };

  window.renderColors = renderColors = function(){
    var box = document.getElementById('colorOptions');
    if(!box || !currentProduct) return;
    box.innerHTML = Object.entries(currentProduct.colors || {}).map(function(entry){
      var c = entry[0];
      var v = entry[1] || {};
      return '<button type="button" class="color-chip ' + (c === selectedColor ? 'active' : '') + '" data-color-name="' + cvEsc(c) + '">' +
        '<span class="color-dot" style="background:' + cvEsc(v.hex || '#ccc') + '"></span>' +
        '<span>' + cvEsc(c) + '</span>' + (v.code ? '<small>' + cvEsc(v.code) + '</small>' : '') +
      '</button>';
    }).join('');
  };
  ...
  window.renderThumbs = renderThumbs = function(){
    ...
    if(!box || !currentProduct) return;
    var imgs = cvImagesForColorOnly(currentProduct, selectedColor);
    if(!selectedImage || imgs.indexOf(selectedImage) === -1) selectedImage = imgs[0] || '';
    box.innerHTML = imgs.map(function(img){
      return '<img src="' + cvEsc(img) + '" data-modal-thumb="' + cvEsc(img) + '" class="' + (img === selectedImage ? 'active' : '') + '" alt="Product image">';
    }).join('');
  };

  window.updateModalImage = updateModalImage = function(){
    var main = document.getElementById('modalImage');
    if(!main || !currentProduct) return;
    var imgs = cvImagesForColorOnly(currentProduct, selectedColor);
    if(!selectedImage || imgs.indexOf(selectedImage) === -1) selectedImage = imgs[0] || '';
    main.src = selectedImage || imgs[0] || 'assets/products/product_01.png';
  };

  window.selectColor = selectColor = function(color){
    selectedColor = cvColorKey(currentProduct, color);
    var imgs = cvImagesForColorOnly(currentProduct, selectedColor);
  ...
  window.selectImage = selectImage = function(img){
    selectedImage = img || selectedImage || '';
    updateModalImage();
    renderThumbs();
  };
  ...
  document.addEventListener('click', function(e){
    var colorBtn = e.target && e.target.closest && e.target.closest('#colorOptions .color-chip[data-color-name]');
    if(colorBtn){
      e.preventDefault();
      e.stopPropagation();
      selectColor(colorBtn.getAttribute('data-color-name'));
      return;
    }
    var thumb = e.target && e.target.closest && e.target.closest('#thumbs [data-modal-thumb]');
    if(thumb){
      e.preventDefault();
      e.stopPropagation();
      selectImage(thumb.getAttribute('data-modal-thumb'));
    }
  }, true);
})();

/* === CV DISCOUNTED PAGE FILTER ONLY - 20260610 ===
   Runs only on discounted-items.html when window.CV_DISCOUNTED_ONLY is true.
   Does not change normal shop viewing.
*/
(function(){
  'use strict';
  if(!window.CV_DISCOUNTED_ONLY) return;
  if(window.__cvDiscountedPageOnlyPatch) return;
  window.__cvDiscountedPageOnlyPatch = true;

  function cvN(v){ var n = Number(v); return Number.isFinite(n) ? n : 0; }
  function cvClean(v){ return String(v || '').trim(); }
  function cvMoney(v){ return 'SAR ' + Math.round(cvN(v)).toLocaleString(); }
  function cvRuleScope(rule){ return String(rule.scope || rule.applyScope || rule.type || 'product').toLowerCase().replace(/\s+/g,''); }
  function cvRulePercent(rule){ return cvN(rule.percent || rule.discountPercent || rule.discount || rule.value || 0); }
  function cvRuleActive(rule){ return rule && rule.active !== false && String(rule.status || 'active').toLowerCase() !== 'inactive' && cvRulePercent(rule) > 0; }
  function cvPageOk(rule){
    var pages = rule && (rule.publishPages || rule.pages || rule.discountPages);
    if(!pages || !Array.isArray(pages) || !pages.length) return true;
    return pages.map(String).some(function(p){ return p.indexOf('discounted-items') !== -1 || p === 'shop' || p === 'shop.html'; });
  }
  function cvRules(p){ return Array.isArray(p && p.discountRules) ? p.discountRules.filter(function(r){ return cvRuleActive(r) && cvPageOk(r); }) : []; }

  function cvRuleMatches(rule, sizeOpt, fabricOpt){
    var scope = cvRuleScope(rule);
    var size = cvClean(sizeOpt && sizeOpt.label);
    var fabric = cvClean(fabricOpt && fabricOpt.label);
    var rSize = cvClean(rule.size || rule.sizeLabel || rule.targetSize);
    var rFabric = cvClean(rule.fabric || rule.fabricLabel || rule.targetFabric);

    if(scope === 'product' || scope === 'all') return true;
    if(scope === 'size') return !!rSize && rSize === size;
    if(scope === 'fabric') return !!rFabric && rFabric === fabric;
    if(scope === 'combo' || scope === 'sizefabric' || scope === 'size+fabric' || scope === 'size_fabric'){
      return !!rSize && !!rFabric && rSize === size && rFabric === fabric;
    }
    return false;
  }

  function cvDiscountPercentFor(p, sizeOpt, fabricOpt){
    var max = cvN(p && p.discountPercent);
    cvRules(p).forEach(function(rule){
      if(cvRuleMatches(rule, sizeOpt, fabricOpt)) max = Math.max(max, cvRulePercent(rule));
    });
    return max;
  }

  function cvAllSelections(p){
    p = typeof normalizeProduct === 'function' ? normalizeProduct(p || {}) : (p || {});
    var sizes = Array.isArray(p.sizeOptions) && p.sizeOptions.length ? p.sizeOptions : [null];
    var fabrics = Array.isArray(p.fabricOptions) && p.fabricOptions.length ? p.fabricOptions : [null];
    var out = [];
    sizes.forEach(function(s){ fabrics.forEach(function(f){ out.push({size:s, fabric:f, discount:cvDiscountPercentFor(p,s,f)}); }); });
    return out;
  }

  function cvHasDiscount(p){
    if(cvN(p && p.discountPercent) > 0) return true;
    return cvAllSelections(p).some(function(x){ return x.discount > 0; });
  }

  function cvFirstDiscountedSelection(p){
    var list = cvAllSelections(p).filter(function(x){ return x.discount > 0; });
    return list[0] || {size:null, fabric:null, discount:cvN(p && p.discountPercent)};
  }

  var oldFinalPrice = typeof finalPrice === 'function' ? finalPrice : null;
  window.finalPrice = finalPrice = function(p, sizeOpt, fabricOpt){
    var incl = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : cvN(p && p.price);
    var d = cvDiscountPercentFor(p, sizeOpt, fabricOpt);
    return Math.round(incl * (1 - d / 100));
  };

  window.priceHTML = priceHTML = function(p, sizeOpt, fabricOpt){
    var before = typeof priceBeforeVat === 'function' ? priceBeforeVat(p, sizeOpt, fabricOpt) : cvN(p && p.price);
    var vat = typeof vatAmount === 'function' ? vatAmount(p, sizeOpt, fabricOpt) : 0;
    var incl = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : before + vat;
    var pct = cvDiscountPercentFor(p, sizeOpt, fabricOpt);
    var after = finalPrice(p, sizeOpt, fabricOpt);
    var html = '<div class="price-detail">' +
      '<div>Selected price before VAT: <strong>' + cvMoney(before) + '</strong></div>' +
      '<div>VAT (' + (typeof vatRate === 'function' ? vatRate(p) : 15) + '%): <strong>' + cvMoney(vat) + '</strong></div>';
    if(pct > 0){
      html += '<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">' + cvMoney(incl) + '</strong></div>' +
        '<div>After discount: <strong class="discount-price" style="color:#c01818;">' + cvMoney(after) + '</strong> <span class="discount-badge" style="color:#c01818;font-weight:800;">' + pct + '% OFF</span></div>';
    }else{
      html += '<div>Total incl. VAT: <strong>' + cvMoney(incl) + '</strong></div>';
    }
    return html + '</div>';
  };

  function cvFilteredDiscountProducts(){
    return (products || []).map(function(p){ return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p; }).filter(cvHasDiscount);
  }

  var oldRenderProducts = typeof renderProducts === 'function' ? renderProducts : null;
  window.renderProducts = renderProducts = function(list){
    var filtered = (list || []).filter(cvHasDiscount);
    if(oldRenderProducts) return oldRenderProducts(filtered);
  };

  window.applySortAndFilter = applySortAndFilter = function(){
    var list = cvFilteredDiscountProducts();
    var sortSelect = document.getElementById('sortSelect');
    var sort = sortSelect ? sortSelect.value : 'discount';
    if(sort === 'priceHigh') list.sort(function(a,b){ return finalPrice(b, cvFirstDiscountedSelection(b).size, cvFirstDiscountedSelection(b).fabric) - finalPrice(a, cvFirstDiscountedSelection(a).size, cvFirstDiscountedSelection(a).fabric); });
    else if(sort === 'priceLow') list.sort(function(a,b){ return finalPrice(a, cvFirstDiscountedSelection(a).size, cvFirstDiscountedSelection(a).fabric) - finalPrice(b, cvFirstDiscountedSelection(b).size, cvFirstDiscountedSelection(b).fabric); });
    else if(sort === 'nameAZ' && typeof displayName === 'function') list.sort(function(a,b){ return displayName(a).localeCompare(displayName(b)); });
    else list.sort(function(a,b){ return cvFirstDiscountedSelection(b).discount - cvFirstDiscountedSelection(a).discount; });
    if(oldRenderProducts) oldRenderProducts(list);
    var title = document.getElementById('shopPageTitle');
    if(title) title.textContent = lang === 'ar' ? 'العروض والخصومات' : 'Discounted Items';
  };

  var oldOpenProduct = typeof openProduct === 'function' ? openProduct : null;
  window.openProduct = openProduct = function(id){
    if(oldOpenProduct) oldOpenProduct(id);
    setTimeout(function(){
      if(!currentProduct) return;
      var pick = cvFirstDiscountedSelection(currentProduct);
      if(pick.size && Array.isArray(currentProduct.sizeOptions)){
        var si = currentProduct.sizeOptions.findIndex(function(s){ return cvClean(s.label) === cvClean(pick.size.label); });
        if(si >= 0){
          selectedSizeOption = currentProduct.sizeOptions[si];
          var sizeSelect = document.getElementById('sizeSelect');
          if(sizeSelect) sizeSelect.value = String(si);
        }
      }
      if(pick.fabric && Array.isArray(currentProduct.fabricOptions)){
        var fi = currentProduct.fabricOptions.findIndex(function(f){ return cvClean(f.label) === cvClean(pick.fabric.label); });
        if(fi >= 0){
          selectedFabricOption = currentProduct.fabricOptions[fi];
          var fabricSelect = document.getElementById('fabricSelect');
          if(fabricSelect) fabricSelect.value = String(fi);
        }
      }
      var price = document.getElementById('modalPrice');
      if(price) price.innerHTML = priceHTML(currentProduct, selectedSizeOption, selectedFabricOption);
      if(typeof updateFabricDescription === 'function') updateFabricDescription();
    }, 0);
  };

  function cvRefreshDiscountedOnly(){
    try{
      products = (products || []).map(function(p){ return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p; });
      if(document.getElementById('productGrid')) applySortAndFilter();
    }catch(e){ console.warn('Discounted page refresh skipped', e); }
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(cvRefreshDiscountedOnly, 500); });
  else setTimeout(cvRefreshDiscountedOnly, 500);
})();

/* === CV EXISTING PAGE CONTENT RUNTIME ONLY FIX ===
   Purpose: show saved Existing Page Builder content on public pages.
   Scope: frontend content display only. Does not change shop product viewing, discount logic, admin, auth, or backend.
*/
(function(){
  'use strict';
  if(window.__cvExistingPageContentRuntimeOnly) return;
  window.__cvExistingPageContentRuntimeOnly = true;
  ...
  function activeLang(){
    try{ return window.lang || localStorage.getItem('lang') || 'en'; }catch(e){ return 'en'; }
  }

  function textFor(page, base){
    var l = activeLang();
    return page[base + '_' + l] || page[base + '_en'] || page[base] || '';
  }
  ...
  async function loadRuntimeSettings(){
    var st = null;
    try{ if(window.settings && Object.keys(window.settings).length) st = window.settings; }catch(e){}
    if(st && st.page_content) return st;
    st = readStoredSettings();
    if(st && st.page_content) return st;
    try{
      var res = await fetch('/api/settings', {cache:'no-store'});
      if(res.ok) return await res.json();
    }catch(e){}
    return st || {};
  }

  function ensureContentSection(title, body, buttonLabel, buttonUrl){
    if(!title && !body && !buttonLabel) return;
    var section = document.getElementById('cvPageBuilderRuntimeContent');
    if(!section){
      section = document.createElement('section');
      section.id = 'cvPageBuilderRuntimeContent';
      section.className = 'intro cv-page-builder-runtime-content';
      var anchor = document.querySelector('.intro') || document.querySelector('.page-hero') || document.querySelector('.hero') || document.querySelector('main') || document.querySelector('header');
    ...
    var html = '';
    if(title) html += '<h2>' + safeHtmlText(title) + '</h2>';
    if(body) html += '<p>' + safeHtmlText(body) + '</p>';
    if(buttonLabel && buttonUrl) html += '<a class="btn primary" href="' + safeHtmlText(buttonUrl).replace(/<br>/g,'') + '">' + safeHtmlText(buttonLabel) + '</a>';
    section.innerHTML = html;
  }

  function applyHomePage(page){
    var title = textFor(page, 'title');
    var subtitle = textFor(page, 'subtitle');
    var body = textFor(page, 'body');
    var buttonLabel = textFor(page, 'button_label');
    var buttonUrl = page.button_url || '';

    setText('heroTitle', title);
    setText('heroText', subtitle);
    if(page.hero_image){
      var hero = document.querySelector('.hero');
      if(hero) hero.style.background = "linear-gradient(90deg,rgba(24,61,50,.88),rgba(24,61,50,.25)), url('" + page.hero_image + "') center/cover";
    }

    if(body){
      var introText = document.getElementById('introText');
      if(introText) introText.textContent = body;
      else ensureContentSection(title, body, buttonLabel, buttonUrl);
    }

    if(buttonLabel && buttonUrl){
      var actions = document.querySelector('.hero-actions');
      if(actions && !document.getElementById('cvPageBuilderHeroButton')){
        var a = document.createElement('a');
        a.id = 'cvPageBuilderHeroButton';
  ...
  function applyGenericPage(page, key){
    var title = textFor(page, 'title');
    var subtitle = textFor(page, 'subtitle');
    var body = textFor(page, 'body') || textFor(page, 'seo_description');
    var buttonLabel = textFor(page, 'button_label');
    var buttonUrl = page.button_url || '';

    var titleEl = document.getElementById('shopPageTitle') || document.querySelector('.page-hero h1') || document.querySelector('main h1') || document.querySelector('h1');
    var subtitleEl = document.querySelector('.page-hero p') || document.querySelector('main > p');
    if(titleEl && title) titleEl.textContent = title;
    if(subtitleEl && subtitle) subtitleEl.textContent = subtitle;

    if(key !== 'shop' && key !== 'discounted') ensureContentSection('', body, buttonLabel, buttonUrl);
    if((key === 'shop' || key === 'discounted') && (title || subtitle)){
      // Keep product grids exactly as they are; only update the visible page heading text.
    }
  }

  async function applyExistingPageContent(){
    var st = await loadRuntimeSettings();
    if(!st || !st.page_content) return;
    var key = currentPageKey();
    var page = st.page_content[key];
    if(!page || page.active === false) return;
    if(key === 'home') applyHomePage(page);
    else applyGenericPage(page, key);
  }

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(applyExistingPageContent, 700); });
  }else{
    setTimeout(applyExistingPageContent, 700);
  }
  window.addEventListener('storage', function(e){ if(e.key === 'cms_settings') setTimeout(applyExistingPageContent, 100); });
})();

/* === CV HOME SHOP-BY-ROOM RUNTIME PERSIST ONLY FIX ===
   Scope: frontend existing home page content only.
   Keeps shop viewing/product rendering untouched.
*/
(function(){
  'use strict';
  if(window.__cvHomeShopRoomRuntimePersistOnly) return;
  window.__cvHomeShopRoomRuntimePersistOnly = true;

  function langNow(){ try{ return window.lang || localStorage.getItem('lang') || 'en'; }catch(e){ return 'en'; } }
  function txt(obj, key){ var l = langNow(); return obj[key + '_' + l] || obj[key + '_en'] || obj[key] || ''; }
  function esc(v){ return String(v ?? '').replace(/[&<>"']/g, function(c){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]; }); }
  function readStored(){
    try{ var raw = localStorage.getItem('cms_settings') || sessionStorage.getItem('cms_settings'); if(raw) return JSON.parse(raw); }catch(e){}
    try{ if(window.settings && window.settings.page_content) return window.settings; }catch(e){}
    return null;
  }
  async function loadSettings(){
    var local = readStored();
    if(local && local.page_content) return local;
    try{ var res = await fetch('/api/settings', {cache:'no-store'}); if(res.ok) return await res.json(); }catch(e){}
    return local || {};
  }
  function page(){
    var st = readStored() || window.settings || {};
    return st.page_content && st.page_content.home ? st.page_content.home : null;
  }
  function ensureRoomSection(p){
    if(!p) return;
    var cards = Array.isArray(p.shop_room_cards) ? p.shop_room_cards : [];
    if(!cards.length && !p.shop_room_title_en && !p.shop_room_title_ar) return;

    var section = document.getElementById('cvShopByRoomSection');
    if(!section){
      var existing = Array.from(document.querySelectorAll('section')).find(function(s){ return /shop by room|تسوق حسب الغرفة/i.test(s.textContent || ''); });
      section = existing || document.createElement('section');
      section.id = 'cvShopByRoomSection';
      section.className = section.className || 'intro cv-shop-by-room-section';
      if(!existing){
        var custom = document.getElementById('custom');
        if(custom && custom.parentNode) custom.parentNode.insertBefore(section, custom);
        else document.body.appendChild(section);
      }
    }
    var title = txt(p, 'shop_room_title') || 'Shop by Room';
    var allText = txt(p, 'shop_room_shop_all') || 'Shop All';
    var allUrl = p.shop_room_shop_all_url || 'shop.html';
    var cardHtml = cards.map(function(c){
      return '<a class="category-card-link" href="' + esc(c.url || 'shop.html') + '"><strong>' + esc(txt(c, 'title')) + '</strong><br><small>' + esc(txt(c, 'text') || 'Explore') + '</small></a>';
    }).join('');
    section.innerHTML = '<div style="display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:18px;"><h2 style="margin:0;">' + esc(title) + '</h2><a href="' + esc(allUrl) + '">' + esc(allText) + '</a></div><div class="home-room-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;">' + cardHtml + '</div>';
  }
  function applyTextContent(p){
    if(!p || p.active === false) return;
    var title = txt(p, 'title'), subtitle = txt(p, 'subtitle'), body = txt(p, 'body');
    var heroTitle = document.getElementById('heroTitle'); if(heroTitle && title) heroTitle.textContent = title;
    var heroText = document.getElementById('heroText'); if(heroText && subtitle) heroText.textContent = subtitle;
    var introText = document.getElementById('introText'); if(introText && body) introText.textContent = body;
    ensureRoomSection(p);
  }
  async function refresh(){
    var st = await loadSettings();
    if(st && st.page_content && st.page_content.home){
      try{ localStorage.setItem('cms_settings', JSON.stringify(st)); }catch(e){}
      applyTextContent(st.page_content.home);
    }
  }
  [100,700,1500,3000,5000].forEach(function(ms){ setTimeout(refresh, ms); });
  document.addEventListener('DOMContentLoaded', function(){ [100,900,2500,4500].forEach(function(ms){ setTimeout(refresh, ms); }); });
  window.addEventListener('storage', function(e){ if(e.key === 'cms_settings') setTimeout(refresh, 100); });
})();
