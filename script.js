...
/* FRONTEND DISCOUNT + PRODUCT ACTION FIX - 20260609
   Frontend only. Does not change admin/backend. Supports product discounts plus size/fabric/combo discountRules saved by admin. */
(function(){
  function cvN(v){ return Number(v || 0) || 0; }
  function cvClean(v){ return String(v == null ? '' : v).trim(); }
  function cvLabel(opt){ return cvClean((opt && (opt.label || opt.name || opt.value)) || opt || ''); }
  function cvRuleMatches(rule, sizeOpt, fabricOpt){
    if(!rule || rule.active === false) return false;
    const scope = cvClean(rule.scope || rule.applyScope || rule.type || 'product').toLowerCase();
    const size = cvLabel(sizeOpt);
    const fabric = cvLabel(fabricOpt);
    const ruleSize = cvClean(rule.size || rule.sizeLabel || '');
    const ruleFabric = cvClean(rule.fabric || rule.fabricLabel || '');
    ...
    if(scope === 'combo' || scope === 'size_fabric' || scope === 'size+fabric') return !!ruleSize && !!ruleFabric && ruleSize === size && ruleFabric === fabric;
    return false;
  }

  window.cvAppliedDiscountPercent = function(p, sizeOpt, fabricOpt){
    p = (typeof normalizeProduct === 'function') ? normalizeProduct(p || {}) : (p || {});
    let pct = cvN(p.discountPercent || p.discount || 0);
    const rules = Array.isArray(p.discountRules) ? p.discountRules : [];
    rules.forEach(function(rule){
      if(cvRuleMatches(rule, sizeOpt, fabricOpt)) pct = Math.max(pct, cvN(rule.percent || rule.discountPercent || rule.discount));
    });
    return Math.max(0, Math.min(90, pct));
  };

  window.finalPrice = finalPrice = function(p, sizeOpt, fabricOpt){
    const incl = (typeof priceIncludingVat === 'function') ? priceIncludingVat(p, sizeOpt, fabricOpt) : cvN(p && p.price);
    const pct = window.cvAppliedDiscountPercent(p, sizeOpt, fabricOpt);
    return Math.round(incl * (1 - pct / 100));
  };

  window.cvLowestVariant = function(p){
    p = (typeof normalizeProduct === 'function') ? normalizeProduct(p || {}) : (p || {});
    const sizes = Array.isArray(p.sizeOptions) && p.sizeOptions.length ? p.sizeOptions : [null];
    const fabrics = Array.isArray(p.fabricOptions) && p.fabricOptions.length ? p.fabricOptions : [null];
    let best = null;
    sizes.forEach(function(s){
      fabrics.forEach(function(f){
        const beforeExVat = (typeof priceBeforeVat === 'function') ? priceBeforeVat(p, s, f) : cvN(p.price);
        const before = Math.round((typeof priceIncludingVat === 'function') ? priceIncludingVat(p, s, f) : beforeExVat);
        const discount = window.cvAppliedDiscountPercent(p, s, f);
        const final = finalPrice(p, s, f);
        const row = {beforeExVat:beforeExVat, before:before, final:final, discount:discount, size:s, fabric:f};
        if(!best || row.final < best.final) best = row;
      });
    });
    return best || {before:cvN(p.price), final:cvN(p.price), discount:0, size:null, fabric:null};
  };

  window.priceHTML = priceHTML = function(p, sizeOpt, fabricOpt){
    p = (typeof normalizeProduct === 'function') ? normalizeProduct(p || {}) : (p || {});
    const before = (typeof priceBeforeVat === 'function') ? priceBeforeVat(p, sizeOpt, fabricOpt) : cvN(p.price);
    const vat = (typeof vatAmount === 'function') ? vatAmount(p, sizeOpt, fabricOpt) : 0;
    const rate = (typeof vatRate === 'function') ? vatRate(p) : 15;
    const incl = Math.round((typeof priceIncludingVat === 'function') ? priceIncludingVat(p, sizeOpt, fabricOpt) : before + vat);
    const pct = window.cvAppliedDiscountPercent(p, sizeOpt, fabricOpt);
    const fp = finalPrice(p, sizeOpt, fabricOpt);
    let html = '<div class="price-detail">' +
      '<div>Selected price before VAT: <strong>' + money(before) + '</strong></div>' +
    ...
    if(pct > 0){
      html += '<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">' + money(incl) + '</strong></div>' +
        '<div>After discount: <strong class="discount-price" style="color:#c62828!important;font-weight:900;">' + money(fp) + '</strong> ' +
        '<span class="discount-badge" style="background:#c62828!important;color:#fff!important;">' + pct + '% OFF</span></div>';
    }else{
      html += '<div>Total incl. VAT: <strong>' + money(incl) + '</strong></div>';
    }
    return html + '</div>';
...
/* CRAFTED-VISUAL-FRONTEND-PRODUCT-VISIBILITY-DISCOUNT-ACTION-FIX-20260609-38
   Frontend only: keep products visible, red discount rendering, and reliable Quick View/Customize buttons.
*/
(function(){
  'use strict';
  if(window.__cv38FrontProductVisibilityPatch) return;
  window.__cv38FrontProductVisibilityPatch = true;

  function n(v){ const x=Number(v); return Number.isFinite(x)?x:0; }
  function esc(v){ return String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function money(v){ try{ if(typeof window.money==='function') return window.money(v); }catch(e){} return 'SAR ' + Math.round(n(v)).toLocaleString(); }
  function clean(v){ return String(v ?? '').trim(); }
  function safeImg(p){ try{ if(typeof firstImage==='function') return firstImage(p); }catch(e){} const colors=p&&p.colors?p.colors:{}; for(const k of Object.keys(colors)){ const set=colors[k]||{}; if(Array.isArray(set.imageMeta)&&set.imageMeta[0]?.url) return set.imageMeta[0].url; if(Array.isArray(set.images)&&set.images[0]) return set.images[0]; } return (Array.isArray(p?.gallery)&&p.gallery[0]) || 'assets/products/product_01.png'; }
  function wished(id){ try{ return typeof isWished==='function' ? isWished(id) : false; }catch(e){ return false; } }
  function displayProductName(p){ try{ return typeof displayName==='function'?displayName(p):(p.name||p.name_en||'Product'); }catch(e){ return p.name||p.name_en||'Product'; } }
  function displayProductCat(p){ try{ return typeof displayCategory==='function'?displayCategory(p):(p.category||p.category_name||''); }catch(e){ return p.category||p.category_name||''; } }
  function normalize(p){ try{ return typeof normalizeProduct==='function'?normalizeProduct(p||{}):(p||{}); }catch(e){ return p||{}; } }
  function vat(p){ try{ return typeof vatRate==='function'?vatRate(p):n(p.vatRate||15); }catch(e){ return n(p.vatRate||15)||15; } }
  function beforeVat(p,s,f){ try{ return typeof priceBeforeVat==='function'?priceBeforeVat(p,s,f):n(p.price||p.base_price); }catch(e){ return n(p.price||p.base_price); } }
  function final(p,s,f){ try{ return typeof finalPrice==='function'?finalPrice(p,s,f):Math.round(beforeVat(p,s,f)*(1+vat(p)/100)*(1-n(p.discountPercent)/100)); }catch(e){ return Math.round(beforeVat(p,s,f)*(1+vat(p)/100)*(1-n(p.discountPercent)/100)); } }
  function lowest(p){
    p=normalize(p);
    const vals=[];
    const sizes=Array.isArray(p.sizeOptions)&&p.sizeOptions.length?p.sizeOptions:[null];
    const fabrics=Array.isArray(p.fabricOptions)&&p.fabricOptions.length?p.fabricOptions:[null];
    sizes.forEach(s=>fabrics.forEach(f=>{ const b=beforeVat(p,s,f); if(b>0) vals.push({before:b, final:final(p,s,f)}); }));
    if(!vals.length) vals.push({before:n(p.price||p.base_price), final:final(p)});
    vals.sort((a,b)=>a.final-b.final);
    return vals[0];
  }
  function priceCard(p){
    const d=n(p.discountPercent||p.discount);
    const low=lowest(p);
    const oldIncl=Math.round(low.before*(1+vat(p)/100));
    if(d>0){
      return `<div class="shop-price-summary"><small>From</small><br><span class="old-price">${money(oldIncl)}</span><br><strong class="discount-price">${money(low.final)}</strong> <span class="discount-badge">${d}% OFF</span></div>`;
    }
    return `<div class="shop-price-summary"><small>From</small><br><strong>${money(low.final)}</strong></div>`;
  }
  ...
  window.renderProducts = renderProducts = function(list){
    ...
    const rows=(Array.isArray(list)?list:[]).map(normalize).filter(p => p && (p.active !== false) && (p.id || p._dbId || p.name));
    if(!rows.length){ grid.innerHTML='<div class="empty-products"><h3>No products showing yet</h3><p>No products are currently published.</p></div>'; return; }
    grid.innerHTML=rows.map(p=>{
      const pid=String(p.id || p._dbId || p.sku || '');
      const img=safeImg(p);
      const name=displayProductName(p);
      const cat=displayProductCat(p);
      const isWish=wished(pid);
      const colors=p.colors&&typeof p.colors==='object'?p.colors:{};
      return `<div class="card product-card" data-product-id="${esc(pid)}"><div class="product-card-image-wrap" style="position:relative;"><img src="${esc(img)}" alt="${esc(name)}" onerror="this.src='assets/products/product_01.png'"><button type="button" class="wishlist-heart ${isWish?'active':''}" data-shop-action="wishlist" data-wishlist-id="${esc(pid)}" aria-label="${isWish?'Remove from wishlist':'Add to wishlist'}">${isWish?'♥':'♡'}</button></div><div class="card-body"><h3>${esc(name)}</h3><p>${esc(cat)}</p>${(typeof starDisplay==='function')?starDisplay(pid):''}${priceCard(p)}<div class="mini-swatches">${Object.entries(colors).map(([c,v])=>`<span title="${esc(c)}" style="background:${esc((v&&v.hex)||'#ccc')}"></span>`).join('')}</div><div class="cj-card-actions"><button class="btn secondary" type="button" data-shop-action="quick" data-product-id="${esc(pid)}">Quick View</button><button class="btn primary" type="button" data-shop-action="customize" data-product-id="${esc(pid)}">Customize</button></div></div></div>`;
    }).join('');
  };

  const oldApply = typeof applySortAndFilter==='function' ? applySortAndFilter : null;
  window.applySortAndFilter = applySortAndFilter = function(){
    try{
      products=(products||[]).map(normalize).filter(p=>p && p.active!==false);
      let list=[...products];
      const catFilter=document.getElementById('categoryFilter');
      let selected=catFilter && catFilter.value ? catFilter.value : (window.CV_CATEGORY_FILTER || activeCategory || 'All');
      if(selected && selected!=='All'){
        const filtered=list.filter(p=>String(p.category||p.category_name||'')===String(selected));
        if(filtered.length) list=filtered;
        else { selected='All'; if(catFilter) catFilter.value='All'; activeCategory='All'; }
      }
      const sort=document.getElementById('sortSelect')?.value || 'featured';
      if(sort==='priceHigh') list.sort((a,b)=>lowest(b).final-lowest(a).final);
      else if(sort==='priceLow') list.sort((a,b)=>lowest(a).final-lowest(b).final);
      else if(sort==='discount') list.sort((a,b)=>n(b.discountPercent)-n(a.discountPercent));
      const title=document.getElementById('shopPageTitle'); if(title && selected && selected!=='All') title.textContent=selected;
      renderProducts(list);
    }catch(e){ console.warn('Safe shop render fallback used.', e); if(oldApply) oldApply(); }
  };

  document.addEventListener('click', function(e){
    const btn=e.target.closest('[data-shop-action]'); if(!btn) return;
    const action=btn.getAttribute('data-shop-action');
    const productId=btn.getAttribute('data-product-id') || btn.getAttribute('data-wishlist-id') || btn.closest('[data-product-id]')?.getAttribute('data-product-id');
    if(action==='quick' || action==='customize'){
      e.preventDefault(); e.stopPropagation();
      if(action==='quick'){
        if(typeof window.openProduct==='function') window.openProduct(productId);
        else if(typeof openProduct==='function') openProduct(productId);
      }else{
        if(typeof window.openCustomizeProduct==='function') window.openCustomizeProduct(productId);
        else if(typeof window.openProduct==='function') window.openProduct(productId);
      }
    }
  }, true);

  try{ setTimeout(()=>{ if(Array.isArray(products) && products.length) window.applySortAndFilter(); }, 300); }catch(e){}
})();
