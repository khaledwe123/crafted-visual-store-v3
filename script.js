...
/* === CV SHOP VISIBILITY ONLY FIX - 20260610 ===
   Scope: shop/frontend visibility only.
   Fixes products published in admin not appearing on shop page.
   Does not change admin, auth, discount, or super-admin logic.
*/
(function(){
  'use strict';
  if(window.__cvShopVisibilityOnly20260610) return;
  window.__cvShopVisibilityOnly20260610 = true;
  ...
  function asArray(payload){
    ...
    const candidates = [payload.products, payload.items, payload.data, payload.rows, payload.results];
    for(const c of candidates){
      if(Array.isArray(c)) return c;
      if(c && typeof c === 'object'){
        const nested = asArray(c);
  ...
  function boolFalse(value){
    return value === false || value === 0 || value === '0' || String(value).toLowerCase() === 'false' || String(value).toLowerCase() === 'no';
  }
  ...
  function isShopVisible(row, product){
    row = row || {};
    product = product || {};
    const merged = Object.assign({}, row, product);

    if(merged.deleted_at || merged.deletedAt || boolTrue(merged.deleted) || boolTrue(merged.is_deleted) || boolTrue(merged.isDeleted) || boolTrue(merged.archived)) return false;

    const status = String(merged.status || merged.product_status || merged.visibility || '').trim().toLowerCase();
    if(['inactive','disabled','draft','unpublished','hidden','private','deleted','archived'].includes(status)) return false;

    const activeKeys = ['active','is_active','isActive','enabled','is_enabled','isEnabled'];
    for(const key of activeKeys){
      if(hasAny(merged, [key]) && boolFalse(merged[key])) return false;
    }

    const publishKeys = ['published','is_published','isPublished','published_to_shop','publishedToShop','show_in_shop','showInShop','visible','is_visible','isVisible'];
    const presentPublishKeys = publishKeys.filter(k => hasAny(merged, [k]));
    if(presentPublishKeys.length){
      return presentPublishKeys.some(k => boolTrue(merged[k]));
    }

    // Legacy records did not always have a publish field; active products remain visible.
    return true;
  }
  ...
  function toProduct(row){
    ...
    const data = Object.assign(
      {},
      parseMaybeJSON(row.data_json),
      parseMaybeJSON(row.data),
      parseMaybeJSON(row.product_data),
      parseMaybeJSON(row.product_json)
    );
    const product = Object.assign({}, data);

    product._dbId = row.id || product._dbId;
    product.id = product.id || product.slug || row.slug || row.sku || String(row.id || Date.now());
    product.slug = product.slug || row.slug || product.id;
    product.name = product.name || row.name_en || row.name || row.title || '';
    product.name_ar = product.name_ar || row.name_ar || row.title_ar || '';
    product.category = product.category || row.category_name || row.category || row.category_en || 'All Products';
    product.category_ar = product.category_ar || row.category_ar || product.category || '';
    product.description = product.description || row.description_en || row.description || '';
    product.description_ar = product.description_ar || row.description_ar || '';
    product.price = Number(product.price || row.base_price || row.price || 0);
    product.vatRate = Number(product.vatRate || row.vat_rate || (window.settings && settings.vat_rate) || 15);

    // Preserve visibility fields for filtering/debugging.
    ['active','is_active','isActive','enabled','published','is_published','isPublished','published_to_shop','publishedToShop','show_in_shop','showInShop','visible','is_visible','isVisible','status','deleted_at','deletedAt','deleted'].forEach(function(k){
      if(product[k] === undefined && row[k] !== undefined) product[k] = row[k];
    });

    if(!product.colors || !Object.keys(product.colors || {}).length){
      const img = product.image || product.main_image || row.image_url || row.image || (Array.isArray(product.gallery) ? product.gallery[0] : '');
      product.colors = { Standard: { hex:'#cccccc', code:'', images: img ? [img] : [] } };
    }
  ...
  async function readBackendProducts(){
    const urls = ['/api/products/shop', '/api/products?published=true', '/api/products'];
      ...
      try{
        ...
        if(rows.length){
          return rows.map(function(row){
            ...
            return isShopVisible(x.row, x.product);
    ...
    [localStorage, sessionStorage].forEach(function(storage){
      keys.forEach(function(key){
        try{
          const raw = storage.getItem(key);
          if(!raw) return;
          asArray(JSON.parse(raw)).forEach(function(row){
            const product = toProduct(row);
            if(isShopVisible(row, product)) output.push(product);
          });
        }catch(e){}
      });
    });
    const seen = new Set();
    return output.filter(function(p){
      const id = String(p.id || p._dbId || p.slug || '');
  ...
  function categoryMatches(product, selected){
    ...
    return [product.category, product.category_ar, product.category_name, product.categoryId, product.category_id]
      .filter(Boolean)
      .some(function(v){ return String(v).trim().toLowerCase() === wanted; });
  }

  window.applySortAndFilter = applySortAndFilter = function(){
    products = (products || []).map(function(p){ return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p; });
    let list = products.filter(function(p){ return isShopVisible(p, p); });

    const catFilter = document.getElementById('categoryFilter');
    const sortSelect = document.getElementById('sortSelect');
    const selectedCategory = catFilter && catFilter.value ? catFilter.value : (window.CV_CATEGORY_FILTER || activeCategory || 'All');
    activeCategory = selectedCategory || 'All';

    list = list.filter(function(p){ return categoryMatches(p, activeCategory); });
    ...
    else if(sort === 'discount') list.sort(function(a,b){ return Number(b.discountPercent||0)-Number(a.discountPercent||0); });
  ...
  async function refreshShopProducts(){
    ...
    let fresh = [];
    if(window.location.protocol === 'http:' || window.location.protocol === 'https:') fresh = await readBackendProducts();
    if(!fresh.length) fresh = readLocalProducts();
    if(fresh.length){
      products = fresh;
      if(typeof renderDynamicCategories === 'function') renderDynamicCategories();
      applySortAndFilter();
  ...
  window.cvRefreshShopProducts = refreshShopProducts;

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ setTimeout(refreshShopProducts, 500); });
  }else{
    setTimeout(refreshShopProducts, 500);
  }

  window.addEventListener('focus', function(){ setTimeout(refreshShopProducts, 150); });
  window.addEventListener('storage', function(e){
    if(['cvPrototypeProducts','adminProducts','cms_products','products'].includes(e.key)) setTimeout(refreshShopProducts, 100);
  });
})();
