...
/* CV SCOPED DISCOUNT + DISCOUNTED ITEMS PAGE ONLY - 20260610
   Scope: keep existing shop rendering/layout as-is.
   Adds support for product discountRules saved from Discount Page and filters only discounted-items.html.
*/
(function(){
  'use strict';
  if(window.__cvScopedDiscountAndDiscountPageOnly20260610) return;
  window.__cvScopedDiscountAndDiscountPageOnly20260610 = true;

  function num(v){ var n = Number(v); return Number.isFinite(n) ? n : 0; }
  function clean(v){ return String(v || '').trim().toLowerCase(); }
  function money(n){ return 'SAR ' + Math.round(num(n)).toLocaleString(); }
  function isDiscountedPage(){
    return !!window.CV_DISCOUNTED_ONLY || /discounted-items\.html$/i.test(String(location.pathname || ''));
  }
  function ruleScope(rule){ return clean(rule.scope || rule.applyScope || rule.type || 'product'); }
  function rulePercent(rule){ return num(rule.percent || rule.discountPercent || rule.discount || rule.value); }
  function ruleActive(rule){ return rule && rule.active !== false && rule.status !== 'inactive' && rule.enabled !== false; }
  function sameLabel(a,b){ return clean(a) === clean(b); }
  function selectedSizeLabel(sizeOpt){ return sizeOpt && (sizeOpt.label || sizeOpt.name || sizeOpt.size) || ''; }
  function selectedFabricLabel(fabricOpt){ return fabricOpt && (fabricOpt.label || fabricOpt.name || fabricOpt.fabric) || ''; }

  function ruleMatches(rule, sizeOpt, fabricOpt){
    if(!ruleActive(rule) || rulePercent(rule) <= 0) return false;
    var scope = ruleScope(rule);
    var wantedSize = rule.size || rule.sizeLabel || rule.targetSize || '';
    var wantedFabric = rule.fabric || rule.fabricLabel || rule.targetFabric || '';
    var size = selectedSizeLabel(sizeOpt);
    var fabric = selectedFabricLabel(fabricOpt);

    if(scope === 'product' || scope === 'all') return true;
    if(scope === 'size') return !!size && sameLabel(size, wantedSize);
    if(scope === 'fabric') return !!fabric && sameLabel(fabric, wantedFabric);
    if(scope === 'combo' || scope === 'size_fabric' || scope === 'size+fabric' || scope === 'sizefabric'){
      return !!size && !!fabric && sameLabel(size, wantedSize) && sameLabel(fabric, wantedFabric);
    }
    return false;
  }

  function discountPercentForVariant(product, sizeOpt, fabricOpt){
    product = product || {};
    var best = num(product.discountPercent || product.discount || 0);
    var rules = Array.isArray(product.discountRules) ? product.discountRules : [];
    rules.forEach(function(rule){
      if(ruleMatches(rule, sizeOpt, fabricOpt)) best = Math.max(best, rulePercent(rule));
    });
    return best;
  }

  function maxDiscountPercent(product){
    product = product || {};
    var best = num(product.discountPercent || product.discount || 0);
    (Array.isArray(product.discountRules) ? product.discountRules : []).forEach(function(rule){
      if(ruleActive(rule)) best = Math.max(best, rulePercent(rule));
    });
    return best;
  }

  function productHasAnyDiscount(product){
    return maxDiscountPercent(product) > 0;
  }

  window.cvDiscountPercentForVariant = discountPercentForVariant;
  window.cvProductHasAnyDiscount = productHasAnyDiscount;

  var baseFinalPrice = typeof finalPrice === 'function' ? finalPrice : null;
  window.finalPrice = finalPrice = function(p, sizeOpt, fabricOpt){
    var incl = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : num(p && p.price);
    var pct = discountPercentForVariant(p, sizeOpt, fabricOpt);
    return Math.round(incl * (1 - pct / 100));
  };

  var basePriceHTML = typeof priceHTML === 'function' ? priceHTML : null;
  window.priceHTML = priceHTML = function(p, sizeOpt, fabricOpt){
    var before = typeof priceBeforeVat === 'function' ? priceBeforeVat(p, sizeOpt, fabricOpt) : num(p && p.price);
    var vat = typeof vatAmount === 'function' ? vatAmount(p, sizeOpt, fabricOpt) : 0;
    var rate = typeof vatRate === 'function' ? vatRate(p) : 15;
    ...
    var html = '<div class="price-detail">' +
      '<div>Selected price before VAT: <strong>' + money(before) + '</strong></div>' +
      '<div>VAT (' + rate + '%): <strong>' + money(vat) + '</strong></div>';
    if(pct > 0){
      html += '<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">' + money(incl) + '</strong></div>' +
        '<div>After discount: <strong class="discount-price" style="color:#c1121f;">' + money(after) + '</strong> <span class="discount-badge" style="color:#c1121f;font-weight:800;">-' + pct + '% OFF</span></div>';
    }else{
      html += '<div>Total incl. VAT: <strong>' + money(incl) + '</strong></div>';
    }
    html += '</div>';
  ...
  var baseApplySortAndFilter = typeof applySortAndFilter === 'function' ? applySortAndFilter : null;
  window.applySortAndFilter = applySortAndFilter = function(){
    if(!isDiscountedPage()){
      if(baseApplySortAndFilter) return baseApplySortAndFilter();
      return;
    }

    var list = (products || []).map(function(p){ return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p; })
      .filter(productHasAnyDiscount);
    var sortSelect = document.getElementById('sortSelect');
    var sort = sortSelect ? sortSelect.value : 'discount';
    if(sort === 'priceHigh') list.sort(function(a,b){ return finalPrice(b)-finalPrice(a); });
    else if(sort === 'priceLow') list.sort(function(a,b){ return finalPrice(a)-finalPrice(b); });
    else if(sort === 'nameAZ' && typeof displayName === 'function') list.sort(function(a,b){ return displayName(a).localeCompare(displayName(b)); });
    else list.sort(function(a,b){ return maxDiscountPercent(b)-maxDiscountPercent(a); });

    var title = document.getElementById('shopPageTitle');
    if(title) title.textContent = lang === 'ar' ? 'العروض والخصومات' : 'Discounted Items';
    var grid = document.getElementById('productGrid');
    if(grid && !list.length){
      grid.innerHTML = '<div class="empty-products"><h3>No discounted products yet</h3><p>Active discounted items will appear here after you apply and save a product discount.</p></div>';
      return;
    }
    if(typeof renderProducts === 'function') renderProducts(list);
  };

  function refreshDiscountedPage(){
    if(!isDiscountedPage()) return;
    try{
      if(typeof renderDynamicCategories === 'function') renderDynamicCategories();
      applySortAndFilter();
  ...
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(refreshDiscountedPage, 600); setTimeout(refreshDiscountedPage, 1600); });
  else { setTimeout(refreshDiscountedPage, 600); setTimeout(refreshDiscountedPage, 1600); }
})();
