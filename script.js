...
/* CRAFTED-VISUAL-SHOP-SIZE-FABRIC-COLOR-DISCOUNT-PATCH-20260614
   Corrected-files-only patch from the last deployed folder.
   Scope: Shop discount calculation/display only. Does not change Quick View, cart, reviews, admin, menu, or styling.
*/
(function(){
  'use strict';
  if(window.__cvShopSizeFabricColorDiscountPatch) return;
  window.__cvShopSizeFabricColorDiscountPatch = true;

  function norm(v){ return String(v || '').trim().toLowerCase(); }
  function labelOf(v){ return String((v && (v.label || v.name || v.value || v.title)) || v || '').trim(); }
  function money(n){ return 'SAR ' + Math.round(Number(n || 0)).toLocaleString(); }
  function safePercent(v){ const n = Number(v || 0); return Number.isFinite(n) && n > 0 ? n : 0; }
  function currentColor(){
    try{ if(typeof selectedColor !== 'undefined' && selectedColor) return String(selectedColor); }catch(e){}
    return String(window.selectedColor || '');
  }
  function activeDiscountRules(p){ return Array.isArray(p && p.discountRules) ? p.discountRules.filter(r => r && r.active !== false && safePercent(r.percent) > 0) : []; }
  function ruleFieldMatches(ruleValue, selectedValue){
    if(!ruleValue) return true;
    if(!selectedValue) return false;
    return norm(ruleValue) === norm(selectedValue);
  }
  function scopedDiscountPercent(p, sizeOpt, fabricOpt, colorName){
    const size = labelOf(sizeOpt);
    const fabric = labelOf(fabricOpt);
    const color = String(colorName || currentColor() || '').trim();
    let best = safePercent(p && p.discountPercent);
    activeDiscountRules(p).forEach(rule => {
      const scope = norm(rule.scope || 'product');
      let matched = false;
      if(scope === 'product') matched = true;
      else if(scope === 'size') matched = ruleFieldMatches(rule.size, size);
      else if(scope === 'fabric') matched = ruleFieldMatches(rule.fabric, fabric);
      else if(scope === 'combo' || scope === 'size_fabric_color' || scope === 'size+fabric+color') matched = ruleFieldMatches(rule.size, size) && ruleFieldMatches(rule.fabric, fabric) && ruleFieldMatches(rule.color, color);
      else matched = ruleFieldMatches(rule.size, size) && ruleFieldMatches(rule.fabric, fabric) && ruleFieldMatches(rule.color, color);
      if(matched) best = Math.max(best, safePercent(rule.percent));
    });
    return best;
  }
  window.cvScopedDiscountPercent = scopedDiscountPercent;

  const oldFinalPrice = typeof finalPrice === 'function' ? finalPrice : null;
  window.finalPrice = finalPrice = function(p, sizeOpt=null, fabricOpt=null){
    const base = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : Number(p && p.price || 0);
    const discount = scopedDiscountPercent(p, sizeOpt, fabricOpt, currentColor());
    return Math.round(base * (1 - discount / 100));
  };

  window.priceHTML = priceHTML = function(p, sizeOpt=null, fabricOpt=null){
    const before = typeof priceBeforeVat === 'function' ? priceBeforeVat(p, sizeOpt, fabricOpt) : Number(p && p.price || 0);
    const vat = typeof vatAmount === 'function' ? vatAmount(p, sizeOpt, fabricOpt) : 0;
    const incl = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : before + vat;
    const discount = scopedDiscountPercent(p, sizeOpt, fabricOpt, currentColor());
    const fp = Math.round(incl * (1 - discount / 100));
    let html = `<div class="price-detail">
      <div>Selected price before VAT: <strong>${money(before)}</strong></div>
      <div>VAT (${typeof vatRate === 'function' ? vatRate(p) : 15}%): <strong>${money(vat)}</strong></div>`;
    if(discount > 0){
      html += `<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">${money(incl)}</strong></div>
        <div>After discount: <strong class="discount-price">${money(fp)}</strong> <span class="discount-badge">-${discount}% OFF</span></div>`;
    }else{
  ...
  function anyScopedDiscount(p){
    let max = safePercent(p && p.discountPercent);
    activeDiscountRules(p).forEach(r => { max = Math.max(max, safePercent(r.percent)); });
    return max;
  }

  const oldSelectColor = typeof selectColor === 'function' ? selectColor : null;
  window.selectColor = selectColor = function(color){
    if(typeof oldSelectColor === 'function') oldSelectColor(color);
    try{ selectedColor = color || selectedColor; }catch(e){ window.selectedColor = color || window.selectedColor; }
    setTimeout(function(){
      const priceEl = document.getElementById('modalPrice');
      const product = (typeof currentProduct !== 'undefined' && currentProduct) ? currentProduct : window.currentProduct;
      if(priceEl && product) priceEl.innerHTML = priceHTML(product, (typeof selectedSizeOption !== 'undefined' ? selectedSizeOption : null), (typeof selectedFabricOption !== 'undefined' ? selectedFabricOption : null));
    }, 30);
  };

  const oldUpdateSelectedFabric = typeof updateSelectedFabric === 'function' ? updateSelectedFabric : null;
  window.updateSelectedFabric = updateSelectedFabric = function(){
    if(typeof oldUpdateSelectedFabric === 'function') oldUpdateSelectedFabric();
    const priceEl = document.getElementById('modalPrice');
    const product = (typeof currentProduct !== 'undefined' && currentProduct) ? currentProduct : window.currentProduct;
    if(priceEl && product) priceEl.innerHTML = priceHTML(product, (typeof selectedSizeOption !== 'undefined' ? selectedSizeOption : null), (typeof selectedFabricOption !== 'undefined' ? selectedFabricOption : null));
  };

  const oldUpdateSelectedSize = typeof updateSelectedSize === 'function' ? updateSelectedSize : null;
  window.updateSelectedSize = updateSelectedSize = function(){
    if(typeof oldUpdateSelectedSize === 'function') oldUpdateSelectedSize();
    const priceEl = document.getElementById('modalPrice');
    const product = (typeof currentProduct !== 'undefined' && currentProduct) ? currentProduct : window.currentProduct;
    if(priceEl && product) priceEl.innerHTML = priceHTML(product, (typeof selectedSizeOption !== 'undefined' ? selectedSizeOption : null), (typeof selectedFabricOption !== 'undefined' ? selectedFabricOption : null));
  };

  const oldRenderProducts = typeof renderProducts === 'function' ? renderProducts : null;
  if(typeof oldRenderProducts === 'function'){
    window.renderProducts = renderProducts = function(list){
      oldRenderProducts(list);
      try{
        (list || products || []).forEach(p => {
          const pct = anyScopedDiscount(p);
          if(!pct) return;
          document.querySelectorAll(`[data-product-id="${CSS.escape(String(p.id || p._dbId || ''))}"] .shop-price-summary`).forEach(box => {
            if(!box.querySelector('.discount-badge')) box.insertAdjacentHTML('beforeend', ` <span class="discount-badge">Up to -${pct}% OFF</span>`);
          });
        });
      }catch(e){}
    };
