...
(function(){
  ...
  const CV_PAGE_CONNECTIONS = [
    {label_en:'Home', label_ar:'الرئيسية', url:'index.html'},
    {label_en:'Shop', label_ar:'المتجر', url:'shop.html'},
    {label_en:'Discounted Items', label_ar:'العروض', url:'discounted-items.html'},
    {label_en:'Custom Order', label_ar:'طلب تفصيل', url:'index.html#custom'},
    {label_en:'Track Order', label_ar:'تتبع الطلب', url:'track-order.html'},
    {label_en:'Contact Us', label_ar:'تواصل معنا', url:'contact.html'},
    {label_en:'My Account', label_ar:'حسابي', url:'account.html'}
  ];

  function pageConnectionByUrl(url){
    return CV_PAGE_CONNECTIONS.find(p => p.url === url) || null;
  }

  function renderPageConnections(){
    const box = q('pageConnectionsList');
    if(!box) return;
    const items = getMenuItems();
    box.innerHTML = CV_PAGE_CONNECTIONS.map(page => {
      const index = items.findIndex(m => String(m.url || '') === page.url);
      const item = index >= 0 ? items[index] : null;
      const connected = !!item;
      const visible = item ? item.visible !== false : false;
      return '<div class="admin-item">'
        + '<div><strong>'+esc(page.label_en)+'</strong> / '+esc(page.label_ar)+'<br>'+esc(page.url)+'<br>'
        + (connected ? 'Connected to menu: '+(visible ? 'Visible' : 'Hidden') : 'Not connected to menu')
        + '</div><div>'
        + '<button type="button" onclick="addPageConnectionByUrl(\''+esc(page.url).replace(/'/g,"\\'")+'\')">'+(connected ? 'Reconnect' : 'Connect')+'</button>'
        + (connected ? '<button type="button" onclick="toggleMenu('+index+')">Show/Hide</button><button type="button" onclick="removeMenu('+index+')">Remove</button>' : '')
        + '<button type="button" onclick="window.open(\''+esc(page.url).replace(/'/g,"\\'")+'\',\'_blank\')">Open</button>'
        + '</div></div>';
    }).join('');
  }

  window.addPageConnectionByUrl = async function(url){
    if(!window.hasAdminPermission('menu','write')) return status('You have read-only access for Page Connections.', true);
    const page = pageConnectionByUrl(url) || {label_en:url, label_ar:'', url};
    const items = getMenuItems();
    const existing = items.find(m => String(m.url || '') === String(page.url));
    if(existing){
      existing.label_en = existing.label_en || page.label_en;
      existing.label_ar = existing.label_ar || page.label_ar;
      existing.visible = true;
    }else{
      items.push({label_en:page.label_en, label_ar:page.label_ar, url:page.url, visible:true});
    }
    setMenuItems(items);
    if(typeof window.renderMenu === 'function') window.renderMenu();
    renderPageConnections();
    try{ await publishSettings({menu:items}); status('Page connection saved and published.'); }
    catch(e){ status('Page connection saved locally, but backend publish failed: '+e.message, true); }
  };

  window.addSelectedPageConnection = async function(){
    const select = q('pageConnectionSelect');
    const custom = val('pageConnectionCustomUrl');
    if(custom){
      const items = getMenuItems();
      if(!window.hasAdminPermission('menu','write')) return status('You have read-only access for Page Connections.', true);
      if(!items.some(m => String(m.url || '') === custom)) items.push({label_en:custom.replace(/\.html.*$/,''), label_ar:'', url:custom, visible:true});
      setMenuItems(items);
      if(q('pageConnectionCustomUrl')) q('pageConnectionCustomUrl').value = '';
      if(typeof window.renderMenu === 'function') window.renderMenu();
      renderPageConnections();
      try{ await publishSettings({menu:items}); status('Custom page connection saved and published.'); }
      catch(e){ status('Custom page connection saved locally, but backend publish failed: '+e.message, true); }
      return;
    }
    if(select) return window.addPageConnectionByUrl(select.value);
  };
...
(function(){
  function bootPageConnections(){
    try{ if(typeof renderPageConnections === 'function') renderPageConnections(); }catch(e){}
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(bootPageConnections, 250); });
  else setTimeout(bootPageConnections, 250);
})();
