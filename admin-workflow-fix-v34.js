...
(function(){
  ...
  function getProductSizesForDiscount(p){
    const out = [];
    if(p && Array.isArray(p.sizeOptions)) p.sizeOptions.forEach(s => out.push(s && (s.label || s.name || s.value) || s));
    if(p && Array.isArray(p.sizes)) p.sizes.forEach(s => out.push(s && (s.label || s.name || s.value) || s));
    return [...new Set(out.map(x=>String(x || '').trim()).filter(Boolean))];
  }

  function getProductFabricsForDiscount(p){
    const out = [];
    if(p && Array.isArray(p.fabricOptions)) p.fabricOptions.forEach(f => out.push(f && (f.label || f.name || f.value) || f));
    if(p && Array.isArray(p.fabrics)) p.fabrics.forEach(f => out.push(f && (f.label || f.name || f.value) || f));
    return [...new Set(out.map(x=>String(x || '').trim()).filter(Boolean))];
  }

  function hideDiscountSelect(id){ const el = q(id); if(el){ el.style.display = 'none'; el.innerHTML = ''; } }
  function showDiscountSelect(id){ const el = q(id); if(el) el.style.display = ''; return el; }
  ...
  async function renderDiscountVariantTargets(){
    ...
    if(applyScope === 'size' || applyScope === 'combo'){
      ...
      sizeEl.innerHTML = sizes.length ? sizes.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('') : '<option value="">No sizes found for this product</option>';
    }else{
      hideDiscountSelect('discountSizeTarget');
    }
    ...
    if(applyScope === 'fabric' || applyScope === 'combo'){
      ...
      fabricEl.innerHTML = fabrics.length ? fabrics.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('') : '<option value="">No fabrics found for this product</option>';
    }else{
      hideDiscountSelect('discountFabricTarget');
    }
  }
  ...
  window.renderDiscountTargets = async function(){
    ...
    try{
      ...
      status('Could not load discount targets: ' + e.message, true);
    }
  };
  ...
  window.applyDiscount = async function(){
    if(typeof window.hasAdminPermission === 'function' && !window.hasAdminPermission('discounts','write')){
      return status('You do not have write access for discounts.', true);
    }

    const selection = getDiscountSelection();
    const percent = num('bulkDiscount', 0);

    if(!selection.type) return status('Please choose a discount target.', true);
    if(selection.type !== 'all' && !selection.target) return status('Please choose a discount target.', true);
    if(percent < 0 || percent > 90) return status('Discount must be between 0 and 90.', true);
    if(selection.type === 'product' && (selection.applyScope === 'size' || selection.applyScope === 'combo') && !selection.size) return status('Please choose a size for this product.', true);
    if(selection.type === 'product' && (selection.applyScope === 'fabric' || selection.applyScope === 'combo') && !selection.fabric) return status('Please choose a fabric for this product.', true);

    try{
      const count = await updateDiscountedProducts(percent, selection);
      status(`Discount applied and published to ${count} product(s).`);
    }catch(e){
      console.error('Apply discount failed', e);
      status('Could not apply discount: ' + e.message, true);
    }
  };
  ...
  window.clearDiscounts = async function(){
    ...
    try{
      ...
      try{ window.products = products; }catch(e){}
      await window.renderDiscountList();
      status(`All product discounts cleared and published (${products.length} product(s)).`);
    }catch(e){
      console.error('Clear discounts failed', e);
      status('Could not clear discounts: ' + e.message, true);
    }
  };
  ...
  window.saveDiscountPage = async function(){
    ...
    try{
      ...
      status('Discount page is synced with the live backend.');
    }catch(e){
      console.error('Save discount page failed', e);
      status('Could not sync discount page: ' + e.message, true);
    }
  };
    ...
    try{
      ...
      products.forEach(p => {
        if(Number(p.discountPercent || 0) > 0){
          active.push({product:p, line:`Product discount: ${Number(p.discountPercent || 0)}%`});
        }
        (Array.isArray(p.discountRules) ? p.discountRules : []).forEach(r => {
          if(r && r.active !== false && Number(r.percent || 0) > 0){
            const details = [];
            if(r.size) details.push('Size: ' + r.size);
            if(r.fabric) details.push('Fabric: ' + r.fabric);
            active.push({product:p, line:`${String(r.scope || '').toUpperCase()} discount: ${Number(r.percent || 0)}%${details.length ? ' | ' + details.join(' | ') : ''}`});
          }
        });
      });
      box.innerHTML = active.length
        ? active.map(item=>`
          <div class="admin-item">
            <div><strong>${esc(item.product.name || item.product.id)}</strong><br>${esc(item.line)}</div>
            <button type="button" data-discount-edit-product="${esc(item.product.id)}">Edit</button>
          </div>
        `).join('')
