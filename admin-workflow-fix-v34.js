...
/* CRAFTED-VISUAL-DISCOUNT-SIZE-FABRIC-COLOR-ADMIN-PATCH-20260614
   Corrected-files-only patch from the last deployed folder.
   Scope: Discount Page only. Adds Color to Specific Product Size + Fabric combination discounts.
*/
(function(){
  'use strict';
  if(window.__cvDiscountSizeFabricColorAdminPatch) return;
  window.__cvDiscountSizeFabricColorAdminPatch = true;

  function q(id){ return document.getElementById(id); }
  function val(id){ return (q(id)?.value || '').trim(); }
  function esc(v){ return String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function show(message, err){ if(typeof window.showAdminStatus === 'function') window.showAdminStatus(message, !!err); else alert(message); }
  ...
  function normalizeProduct(row){
    let data = row && (row.data || row.data_json) || {};
    if(typeof data === 'string'){ try{ data = JSON.parse(data); }catch(e){ data = {}; } }
    const p = Object.assign({}, data || {});
    p._dbId = row.id || p._dbId;
    p.id = p.id || row.sku || String(row.id || '');
    p.name = p.name || row.name_en || '';
    p.name_ar = p.name_ar || row.name_ar || '';
    p.category = p.category || row.category_name || '';
  ...
  function productPayload(p){
    return {sku:p.id,name_en:p.name,name_ar:p.name_ar||'',category_name:p.category||'',category_ar:p.category_ar||'',description_en:p.description||'',description_ar:p.description_ar||'',base_price:Number(p.price||0),vat_rate:Number(p.vatRate||15),active:true,data:p};
  }
  async function loadProducts(){
    const rows = await publicApi('/products');
    return Array.isArray(rows) ? rows.map(normalizeProduct) : [];
  }
  function ensureColorSelect(){
    let colorEl = q('discountColorTarget');
    const fabricEl = q('discountFabricTarget');
    if(!colorEl && fabricEl){
      colorEl = document.createElement('select');
      colorEl.id = 'discountColorTarget';
      colorEl.style.display = 'none';
      fabricEl.insertAdjacentElement('afterend', colorEl);
    }
    return colorEl;
  }
  function hide(id){ const el=q(id); if(!el) return; el.style.display='none'; if(id !== 'discountApplyScope') el.innerHTML=''; }
  function showEl(id){ const el=q(id); if(el) el.style.display=''; return el; }
  function namesFromColors(p){
    const out = [];
    const colors = p && p.colors && typeof p.colors === 'object' ? p.colors : {};
    Object.keys(colors).forEach(k => { if(String(k).trim()) out.push(String(k).trim()); });
    ['colorOptions','colorsList','colorList'].forEach(k => {
      const arr = p && Array.isArray(p[k]) ? p[k] : [];
      arr.forEach(c => {
        const name = typeof c === 'object' ? (c.label || c.name || c.color || c.title || '') : c;
  ...
  function getSizes(p){
    const out=[]; const add=v=>{ if(v==null) return; if(typeof v==='object') v=v.label||v.name||v.value||v.size||v.title||''; String(v).split(',').forEach(x=>{x=x.trim(); if(x) out.push(x);}); };
    (p.sizeOptions||[]).forEach(add); (p.sizes||[]).forEach(add);
    (p.fabricOptions||[]).forEach(f=>{ Object.keys(f.sizePrices||{}).forEach(add); Object.keys(f.costPrices||{}).forEach(add); });
    Object.keys(p.sizeFabricPrices||{}).forEach(add); Object.keys(p.priceMatrix||{}).forEach(add);
    return [...new Set(out)];
  }
  function getFabrics(p){
    const out=[]; const add=v=>{ if(v==null) return; if(typeof v==='object') v=v.label||v.name||v.value||v.fabric||v.title||''; String(v).split(',').forEach(x=>{x=x.trim(); if(x) out.push(x);}); };
    (p.fabricOptions||[]).forEach(add); (p.fabrics||[]).forEach(add);
    Object.values(p.sizeFabricPrices||{}).forEach(row=>{ if(row&&typeof row==='object') Object.keys(row).forEach(add); });
    Object.values(p.priceMatrix||{}).forEach(row=>{ if(row&&typeof row==='object') Object.keys(row).forEach(add); });
    return [...new Set(out)];
  }
  function ensureScopeOptions(){
    const el=q('discountApplyScope'); if(!el) return;
    const current = el.value || 'product';
    el.innerHTML = [
      '<option value="product">Product Only</option>',
      '<option value="size">Size Only</option>',
  ...
  const oldRenderTargets = window.renderDiscountTargets;
  window.renderDiscountTargets = async function(){
    if(typeof oldRenderTargets === 'function') await oldRenderTargets();
    ensureColorSelect();
    await window.renderDiscountVariantTargets?.();
  };
  ...
  window.renderDiscountVariantTargets = async function(){
    ...
    const productId = val('discountProductTarget');
    const applyScope = val('discountApplyScope') || 'product';
    const scopeEl=q('discountApplyScope'), sizeEl=q('discountSizeTarget'), fabricEl=q('discountFabricTarget'), colorEl=q('discountColorTarget');
    if(!scopeEl || !sizeEl || !fabricEl || !colorEl) return;
    if(type !== 'product'){
      hide('discountApplyScope'); hide('discountSizeTarget'); hide('discountFabricTarget'); hide('discountColorTarget'); return;
    }
    ensureScopeOptions(); showEl('discountApplyScope');
    const products = await loadProducts();
    const p = products.find(x => String(x.id) === String(productId) || String(x._dbId) === String(productId)) || {};
    const sizes = getSizes(p), fabrics = getFabrics(p), colors = namesFromColors(p);
    const selectedSize=sizeEl.value, selectedFabric=fabricEl.value, selectedColor=colorEl.value;
    if(applyScope === 'size' || applyScope === 'combo'){
      showEl('discountSizeTarget'); sizeEl.innerHTML = sizes.length ? '<option value="">Choose Size</option>' + sizes.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('') : '<option value="">No sizes found for this product</option>'; if(selectedSize) sizeEl.value=selectedSize;
    }else hide('discountSizeTarget');
    if(applyScope === 'fabric' || applyScope === 'combo'){
      showEl('discountFabricTarget'); fabricEl.innerHTML = fabrics.length ? '<option value="">Choose Fabric</option>' + fabrics.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('') : '<option value="">No fabrics found for this product</option>'; if(selectedFabric) fabricEl.value=selectedFabric;
    }else hide('discountFabricTarget');
    if(applyScope === 'combo'){
      showEl('discountColorTarget'); colorEl.innerHTML = colors.length ? '<option value="">Choose Color</option>' + colors.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('') : '<option value="">No colors found for this product</option>'; if(selectedColor) colorEl.value=selectedColor;
    }else hide('discountColorTarget');
  };

  function selectedPages(){ return typeof window.getSelectedPublishPages === 'function' ? window.getSelectedPublishPages('discountPublishPages') : ['discounted-items.html']; }
  function ruleMatches(rule, selection){
    return String(rule.scope || '') === String(selection.applyScope || '') && String(rule.size || '') === String(selection.size || '') && String(rule.fabric || '') === String(selection.fabric || '') && String(rule.color || '') === String(selection.color || '');
  }
  function getSelection(){
    const type = val('discountTargetType');
    return {
      type: type || (val('discountScope') || 'product'),
      target: type === 'category' ? val('discountCategoryTarget') : (type === 'product' ? val('discountProductTarget') : 'all'),
      applyScope: type === 'product' ? (val('discountApplyScope') || 'product') : 'product',
      size: val('discountSizeTarget'),
      fabric: val('discountFabricTarget'),
      color: val('discountColorTarget'),
      active: val('discountStatus') !== 'inactive',
      publishPages: selectedPages()
    };
  }
  window.applyDiscount = async function(){
    ...
    try{
      ...
      const changed = [];
      products.forEach(p => {
        const match = selection.type === 'all' || (selection.type === 'category' && String(p.category) === String(selection.target)) || (selection.type === 'product' && (String(p.id) === String(selection.target) || String(p._dbId) === String(selection.target)));
        if(!match) return;
        if(selection.type === 'product' && selection.applyScope !== 'product'){
          const rules = Array.isArray(p.discountRules) ? p.discountRules.filter(r => !ruleMatches(r, selection)) : [];
          if(selection.active && pct > 0){
            rules.push({scope:selection.applyScope,size:(selection.applyScope==='size'||selection.applyScope==='combo')?selection.size:'',fabric:(selection.applyScope==='fabric'||selection.applyScope==='combo')?selection.fabric:'',color:selection.applyScope==='combo'?selection.color:'',percent:pct,active:true,updatedAt:new Date().toISOString(),publishPages:selection.publishPages||['discounted-items.html']});
          }
          p.discountRules = rules;
        }else{
          p.discountPercent = pct;
      ...
      for(const p of changed){ if(p._dbId) await api('/products/' + encodeURIComponent(p._dbId), {method:'PUT', body:productPayload(p)}); }
      try{ window.products = products; localStorage.setItem('cvPrototypeProducts', JSON.stringify(products)); localStorage.setItem('adminProducts', JSON.stringify(products)); sessionStorage.setItem('cvPrototypeProducts', JSON.stringify(products)); }catch(e){}
      if(typeof window.renderDiscountList === 'function') await window.renderDiscountList();
      show(`Discount applied and published to ${changed.length} product(s).`);
    }catch(e){ console.error('Apply discount failed', e); show('Could not apply discount: ' + e.message, true); }
  };

  const oldRenderList = window.renderDiscountList;
  window.renderDiscountList = async function(){
    if(typeof oldRenderList === 'function') await oldRenderList();
    document.querySelectorAll('#discountList .admin-item').forEach(item => {
  ...
  document.addEventListener('change', function(e){
    if(!e.target) return;
    if(e.target.id === 'discountProductTarget' || e.target.id === 'discountApplyScope') setTimeout(()=>window.renderDiscountVariantTargets?.(), 0);
  }, true);
  document.addEventListener('DOMContentLoaded', function(){ ensureColorSelect(); setTimeout(()=>window.renderDiscountVariantTargets?.(), 300); });
})();
