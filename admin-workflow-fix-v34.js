...
(function(){
  ...
  /* === CV MENU EDIT ONLY PATCH ===
     Adds Edit capability to existing Menu Control without changing show/hide/delete behavior.
  */
  let cvEditingMenuIndex = -1;

  function menuFormButton(){
    return Array.from(document.querySelectorAll('button')).find(btn => {
      const raw = btn.getAttribute('onclick') || '';
      return raw.includes('addMenuItem');
    });
  }

  function setMenuEditMode(index){
    cvEditingMenuIndex = Number.isInteger(index) ? index : -1;
    const btn = menuFormButton();
    if(btn) btn.textContent = cvEditingMenuIndex >= 0 ? 'Update Menu Item' : 'Add Menu Item';
    let cancel = document.getElementById('cvMenuCancelEditBtn');
    if(cvEditingMenuIndex >= 0){
      if(!cancel && btn){
        cancel = document.createElement('button');
        cancel.type = 'button';
        cancel.id = 'cvMenuCancelEditBtn';
        cancel.textContent = 'Cancel Edit';
        cancel.style.marginLeft = '8px';
        cancel.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); window.cancelMenuEdit(); });
        btn.insertAdjacentElement('afterend', cancel);
      }
    }else if(cancel){
      cancel.remove();
  ...
  window.editMenuItem = function(i){
    if(!window.hasAdminPermission('menu','write')) return status('You have read-only access for Menu.', true);
    const items = getMenuItems();
    ...
    setMenuEditMode(Number(i));
    status('Editing menu item. Update the fields, then click Update Menu Item.');
  };
  ...
  window.addMenuItem = async function(){
    if(cvEditingMenuIndex < 0){
      return cvMenuOriginalAddMenuItem ? cvMenuOriginalAddMenuItem() : undefined;
    }
    if(!window.hasAdminPermission('menu','write')) return status('You have read-only access for Menu.', true);
    const en = val('menu_label_en');
    const ar = val('menu_label_ar');
    ...
    if(!en || !url) return status('Add menu label and URL.', true);
    const items = getMenuItems();
    if(!items[cvEditingMenuIndex]){
      setMenuEditMode(-1);
      return status('Menu item not found. Please try again.', true);
    }
    const current = items[cvEditingMenuIndex];
    items[cvEditingMenuIndex] = Object.assign({}, current, {label_en:en, label_ar:ar, url:url, visible: current.visible !== false});
    setMenuItems(items);
    clearMenuForm();
    setMenuEditMode(-1);
    if(typeof window.renderMenu === 'function') window.renderMenu();
    try{
      ...
      status('Menu item updated locally, but backend publish failed: '+e.message, true);
    }
  };
  ...
  window.renderMenu = function(){
    ...
    list.innerHTML = items.map((m,i)=>`
      <div class="admin-item">
        <div><strong>${esc(m.label_en || '')}</strong> / ${esc(m.label_ar || '')}<br>${esc(m.url || '')}<br>Visible: ${m.visible !== false}</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <button type="button" onclick="toggleMenu(${i})">Show/Hide</button>
          <button type="button" data-menu-edit-index="${i}">Edit</button>
          <button type="button" onclick="removeMenu(${i})">Delete</button>
        </div>
      </div>`).join('');
    list.querySelectorAll('[data-menu-edit-index]').forEach(btn=>{
      btn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation();
        window.editMenuItem(Number(this.getAttribute('data-menu-edit-index')));
      });
    });
  };
