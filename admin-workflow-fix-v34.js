...
/* === CV HOME SHOP-BY-ROOM EDITOR ONLY FIX ===
   Scope: Page Builder existing Home content fields only.
   Adds editable Shop by Room section fields without changing shop/product viewing.
*/
(function(){
  'use strict';
  if(window.__cvHomeShopByRoomEditorOnly) return;
  window.__cvHomeShopByRoomEditorOnly = true;

  function q(id){ return document.getElementById(id); }
  function val(id){ return (q(id)?.value || '').trim(); }
  function esc(v){ return String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function getSettings(){ try{ if(typeof settings !== 'undefined') return settings || {}; }catch(e){} window.settings = window.settings || {}; return window.settings; }

  const DEFAULT_HOME_ROOMS = [
    {title_en:'Living Room', title_ar:'غرفة المعيشة', text_en:'Explore', text_ar:'استكشف', url:'shop.html?room=Living%20Room'},
    {title_en:'Bedroom', title_ar:'غرفة النوم', text_en:'Explore', text_ar:'استكشف', url:'shop.html?room=Bedroom'},
    {title_en:'Majlis', title_ar:'مجلس', text_en:'Explore', text_ar:'استكشف', url:'shop.html?room=Majlis'},
    {title_en:'Custom Made', title_ar:'تفصيل خاص', text_en:'Explore', text_ar:'استكشف', url:'index.html#custom'}
  ];

  function homeStored(){
    const st = getSettings();
    st.page_content = st.page_content || {};
    return st.page_content.home || {};
  }

  function ensureHomeSectionEditor(){
    if(q('cvHomeShopRoomEditor')) return;
    const anchor = q('page_seo_title_en') || q('page_body_ar') || q('page_body_en');
    if(!anchor) return;
    const wrap = document.createElement('div');
    wrap.id = 'cvHomeShopRoomEditor';
    wrap.style.cssText = 'margin:18px 0;padding:16px;border:1px solid #e4d8c7;border-radius:16px;background:#fffaf2;';
    wrap.innerHTML = `
      <h3 style="margin:0 0 10px;">Home Page — Shop by Room Section</h3>
      <div class="admin-grid-3" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;">
      ...
      <div id="homeRoomCardsEditor" style="margin-top:12px;display:grid;gap:10px;"></div>
    `;
    const parent = anchor.closest('section,.admin-card,form,div') || anchor.parentNode;
    if(parent) parent.insertBefore(wrap, anchor.closest('h3') || anchor);
    renderRoomCardFields();
  }

  function renderRoomCardFields(){
    const box = q('homeRoomCardsEditor');
    if(!box) return;
    box.innerHTML = DEFAULT_HOME_ROOMS.map((r,i)=>`
      <div style="display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;align-items:center;">
        <strong>Card ${i+1}</strong>
        <input id="home_room_${i}_title_en" placeholder="Card title English">
        <input id="home_room_${i}_title_ar" placeholder="Card title Arabic">
        <input id="home_room_${i}_text_en" placeholder="Small text English">
        <input id="home_room_${i}_url" placeholder="Card URL">
      </div>
      <div style="display:grid;grid-template-columns:1fr;gap:8px;margin-top:-6px;">
        <input id="home_room_${i}_text_ar" placeholder="Small text Arabic">
      </div>
    `).join('');
  }

  function fillHomeFields(page){
    ensureHomeSectionEditor();
    page = page || homeStored();
    const rooms = Array.isArray(page.shop_room_cards) && page.shop_room_cards.length ? page.shop_room_cards : DEFAULT_HOME_ROOMS;
    if(q('home_room_title_en')) q('home_room_title_en').value = page.shop_room_title_en || 'Shop by Room';
    if(q('home_room_title_ar')) q('home_room_title_ar').value = page.shop_room_title_ar || 'تسوق حسب الغرفة';
    if(q('home_room_shop_all_en')) q('home_room_shop_all_en').value = page.shop_room_shop_all_en || 'Shop All';
    if(q('home_room_shop_all_ar')) q('home_room_shop_all_ar').value = page.shop_room_shop_all_ar || 'تسوق الكل';
    if(q('home_room_shop_all_url')) q('home_room_shop_all_url').value = page.shop_room_shop_all_url || 'shop.html';
    DEFAULT_HOME_ROOMS.forEach((d,i)=>{
      const r = rooms[i] || d;
      if(q(`home_room_${i}_title_en`)) q(`home_room_${i}_title_en`).value = r.title_en || d.title_en;
      if(q(`home_room_${i}_title_ar`)) q(`home_room_${i}_title_ar`).value = r.title_ar || d.title_ar;
      if(q(`home_room_${i}_text_en`)) q(`home_room_${i}_text_en`).value = r.text_en || d.text_en;
      if(q(`home_room_${i}_text_ar`)) q(`home_room_${i}_text_ar`).value = r.text_ar || d.text_ar;
      if(q(`home_room_${i}_url`)) q(`home_room_${i}_url`).value = r.url || d.url;
    });
  }

  function collectHomeExtras(page){
    page = page || {};
    if(!q('cvHomeShopRoomEditor')) return page;
    page.shop_room_title_en = val('home_room_title_en') || 'Shop by Room';
    page.shop_room_title_ar = val('home_room_title_ar') || 'تسوق حسب الغرفة';
    page.shop_room_shop_all_en = val('home_room_shop_all_en') || 'Shop All';
    page.shop_room_shop_all_ar = val('home_room_shop_all_ar') || 'تسوق الكل';
    page.shop_room_shop_all_url = val('home_room_shop_all_url') || 'shop.html';
    page.shop_room_cards = DEFAULT_HOME_ROOMS.map((d,i)=>({
      title_en: val(`home_room_${i}_title_en`) || d.title_en,
      title_ar: val(`home_room_${i}_title_ar`) || d.title_ar,
      text_en: val(`home_room_${i}_text_en`) || d.text_en,
      text_ar: val(`home_room_${i}_text_ar`) || d.text_ar,
      url: val(`home_room_${i}_url`) || d.url
    }));
    return page;
  }

  const oldEditExisting = window.editExistingPageContent;
  window.editExistingPageContent = function(id){
    const result = typeof oldEditExisting === 'function' ? oldEditExisting.apply(this, arguments) : undefined;
    setTimeout(function(){
      if(String(id) === 'home') fillHomeFields(homeStored());
      else {
        const box = q('cvHomeShopRoomEditor');
  ...
  const oldSaveCustomPage = window.saveCustomPage;
  window.saveCustomPage = async function(){
    const editId = val('page_edit_id');
    if(editId === 'builtin:home'){
      const st = getSettings();
      st.page_content = st.page_content || {};
      st.page_content.home = collectHomeExtras(Object.assign({}, st.page_content.home || {}));
      try{ localStorage.setItem('cms_settings', JSON.stringify(st)); sessionStorage.setItem('cms_settings', JSON.stringify(st)); }catch(e){}
    }
    return typeof oldSaveCustomPage === 'function' ? oldSaveCustomPage.apply(this, arguments) : undefined;
  };

  function boot(){
    ensureHomeSectionEditor();
    const editId = val('page_edit_id');
    if(editId === 'builtin:home') fillHomeFields(homeStored());
  }
