...
if(categories.length){
  const ins = db.prepare('INSERT OR IGNORE INTO categories(name_en,name_ar,active,sort_order) VALUES(?,?,?,?)');
...
if(products.length){
  ...
  const ins = db.prepare('INSERT OR IGNORE INTO products(sku,name_en,name_ar,category_id,description_en,description_ar,base_price,vat_rate,active,data_json) VALUES(?,?,?,?,?,?,?,?,?,?)');
