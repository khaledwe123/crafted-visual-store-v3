/*
  Crafted Visual Admin Workflow Fix v21
  Focus: products inner controls, publishing, media-library image source, analytics period sync.
*/
(function(){
  'use strict';
  ...
  function bindInlineActions(){
    document.addEventListener('click', function(e){
      ...
      const safe = /^(addManualSize|addManualFabric|buildSizeFabricPriceTable|addColorSet|clearForm|saveMenu|saveSettings|saveCategories|saveSeoPage|loadAnalyticsCenter)$/.test(m[1]);
      if(!safe) return;
  ...
  document.addEventListener('DOMContentLoaded', function(){ normalizeSession(); bindInlineActions(); installMediaButtons(); patchAnalytics(); });
  setTimeout(function(){ normalizeSession(); installMediaButtons(); patchAnalytics(); }, 500);
})();
