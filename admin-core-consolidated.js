...
(function(){
  ...
  window.__cvDiscountSaveColorFinalFix = true;

  let editKey = null;

  function q(id){ return document.getElementById(id); }
  function val(id){ return (q(id)?.value || '').trim(); }
  function num(id, fallback){ const n = Number(val(id)); return Number.isFinite(n) ? n : (fallback || 0); }
  function esc(v){ return String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function show(message, isError){ if(typeof window.showAdminStatus === 'function') window.showAdminStatus(message, !!isError); else alert(message); }
  function token(){ return localStorage.getItem('cvAdminApiToken') || sessionStorage.getItem('cvAdminApiToken') || ''; }

  async function adminApi(path, options={}){
    if(window.CV_API && typeof window.CV_API.request === 'function'){
      return window.CV_API.request(path, Object.assign({}, options, {admin:true}));
    }
    const headers = Object.assign({}, options.headers || {});
    const t = token();
    if(t && t !== 'cookie-auth' && t !== 'cv-cookie-auth') headers.Authorization = 'Bearer ' + t;
    ...
    const url = String(path).startsWith('/api/') ? path : '/api/' + String(path).replace(/^\/+/, '').replace(/^api\//,'');
    const res = await fetch(url, {method:options.method || (body === undefined ? 'GET' : 'POST'), credentials:'same-origin', cache:'no-store', headers, body});
    const text = await res.text();
    let data = {};
    try{ data = text ? JSON.parse(text) : {}; }catch(e){ data = text; }
    if(!res.ok) throw new Error((data && data.error) || (typeof data === 'string' && data) || ('HTTP ' + res.status));
  ...
  async function loadCategories(products){
    ...
    try{
      const rows = await publicApi('/categories');
      (Array.isArray(rows) ? rows : []).forEach(c => out.push(c.name_en || c.label_en || c.name || c.category || ''));
    }catch(e){}
    (products || []).forEach(p => out.push(p.category || ''));
    try{ (window.categories || []).forEach(c => out.push(c.label_en || c.name_en || c.name || '')); }catch(e){}
    return [...new Set(out.map(x=>String(x).trim()).filter(Boolean))];
  }

  function addUnique(out, v){
    if(v === undefined || v === null) return;
    if(typeof v === 'object') v = v.label || v.name || v.value || v.size || v.fabric || v.color || v.title || '';
    String(v).split(',').forEach(x => { const t = x.trim(); if(t && !out.includes(t)) out.push(t); });
  }
  function productSizes(p){
    const out=[];
    (p?.sizeOptions || []).forEach(v => addUnique(out, v));
    (p?.sizes || []).forEach(v => addUnique(out, v));
    (p?.fabricOptions || []).forEach(f => {
      Object.keys(f?.sizePrices || {}).forEach(v => addUnique(out, v));
      Object.keys(f?.costPrices || {}).forEach(v => addUnique(out, v));
      Object.keys(f?.sizeCosts || {}).forEach(v => addUnique(out, v));
    });
    Object.keys(p?.sizeFabricPrices || p?.priceMatrix || {}).forEach(v => addUnique(out, v));
    return out;
  }
  function productFabrics(p){
    const out=[];
    (p?.fabricOptions || []).forEach(v => addUnique(out, v));
    (p?.fabrics || []).forEach(v => addUnique(out, v));
    Object.values(p?.sizeFabricPrices || p?.priceMatrix || {}).forEach(row => { if(row && typeof row === 'object') Object.keys(row).forEach(v => addUnique(out, v)); });
    return out;
  }
  function productColors(p){
    const out=[];
    if(p && p.colors && typeof p.colors === 'object'){
      if(Array.isArray(p.colors)) p.colors.forEach(v => addUnique(out, v));
      else Object.keys(p.colors).forEach(v => addUnique(out, v));
    }
    (p?.colorOptions || []).forEach(v => addUnique(out, v));
    (p?.colorList || []).forEach(v => addUnique(out, v));
    (p?.colours || []).forEach(v => addUnique(out, v));
    return out;
  }

  function ensureColorControl(){
    if(q('discountColorTarget')) return q('discountColorTarget');
    const fabric = q('discountFabricTarget');
    const select = document.createElement('select');
    select.id = 'discountColorTarget';
    select.style.display = 'none';
    if(fabric && fabric.parentElement) fabric.insertAdjacentElement('afterend', select);
    return select;
  }
  function setVisible(id, visible){
    const el = id === 'discountColorTarget' ? ensureColorControl() : q(id);
    if(!el) return;
    el.style.display = visible ? '' : 'none';
    if(!visible && id !== 'discountApplyScope') el.innerHTML = '';
  }
  ...
  function ensureScopeOptions(){
    ...
    const options = [
      ...
      ['fabric','Fabric Only'],
      ['color','Color Only'],
      ['combo','Size + Fabric Combination'],
      ['combo_color','Size + Fabric + Color Combination']
    ];
    el.innerHTML = options.map(o => optionHtml(o[0], o[1], o[0] === current)).join('');
    if(!options.some(o => o[0] === current)) el.value = 'product';
  }

  function findProduct(products, id){ return (products || []).find(p => String(p.id) === String(id) || String(p._dbId) === String(id)); }

  window.renderDiscountVariantTargets = async function(){
    ensureColorControl();
    ensureScopeOptions();
    const type = val('discountTargetType');
    const productId = val('discountProductTarget');
    const scope = val('discountApplyScope') || 'product';
    if(type !== 'product'){
      setVisible('discountApplyScope', false); setVisible('discountSizeTarget', false); setVisible('discountFabricTarget', false); setVisible('discountColorTarget', false);
      return;
    }
    setVisible('discountApplyScope', true);
    const products = await loadProducts();
    const p = findProduct(products, productId);
    const sizeEl = q('discountSizeTarget');
    const fabricEl = q('discountFabricTarget');
    const colorEl = ensureColorControl();
    const oldSize = sizeEl?.value || '';
    const oldFabric = fabricEl?.value || '';
    const oldColor = colorEl?.value || '';
    const needsSize = ['size','combo','combo_color'].includes(scope);
    const needsFabric = ['fabric','combo','combo_color'].includes(scope);
    const needsColor = ['color','combo_color'].includes(scope);
    setVisible('discountSizeTarget', needsSize);
    setVisible('discountFabricTarget', needsFabric);
    setVisible('discountColorTarget', needsColor);
    if(needsSize && sizeEl){ const values = p ? productSizes(p) : []; sizeEl.innerHTML = '<option value="">Choose Size</option>' + values.map(v => optionHtml(v, v, v === oldSize)).join(''); if(oldSize) sizeEl.value = oldSize; }
    if(needsFabric && fabricEl){ const values = p ? productFabrics(p) : []; fabricEl.innerHTML = '<option value="">Choose Fabric</option>' + values.map(v => optionHtml(v, v, v === oldFabric)).join(''); if(oldFabric) fabricEl.value = oldFabric; }
    if(needsColor && colorEl){ const values = p ? productColors(p) : []; colorEl.innerHTML = '<option value="">Choose Color</option>' + values.map(v => optionHtml(v, v, v === oldColor)).join(''); if(oldColor) colorEl.value = oldColor; }
  };
  ...
  window.renderDiscountTargets = async function(){
    ...
    const categoryEl = q('discountCategoryTarget');
    const productEl = q('discountProductTarget');
    setVisible('discountCategoryTarget', false);
    setVisible('discountProductTarget', false);
    setVisible('discountApplyScope', false);
    setVisible('discountSizeTarget', false);
    setVisible('discountFabricTarget', false);
    setVisible('discountColorTarget', false);
    if(!type) return;
    try{
      const products = await loadProducts();
      if(type === 'category' && categoryEl){
        const current = categoryEl.value;
        const cats = await loadCategories(products);
        setVisible('discountCategoryTarget', true);
        categoryEl.innerHTML = '<option value="">Choose product category</option>' + cats.map(c => optionHtml(c, c, c === current)).join('');
      }
      if(type === 'product' && productEl){
        const current = productEl.value;
        setVisible('discountProductTarget', true);
        productEl.innerHTML = '<option value="">Choose specific product</option>' + products.map(p => optionHtml(p.id, p.name || p.id, String(p.id) === String(current))).join('');
        if(current) productEl.value = current;
        await window.renderDiscountVariantTargets();
      }
    }catch(e){ console.error('Discount target load failed', e); show('Could not load discount targets: ' + e.message, true); }
  };

  function selectedPublishPages(){
    if(typeof window.getSelectedPublishPages === 'function') return window.getSelectedPublishPages('discountPublishPages');
    return ['discounted-items.html'];
  }

  function getSelection(){
    const type = val('discountTargetType');
    const scope = type === 'product' ? (val('discountApplyScope') || 'product') : 'product';
    return {
      type,
      target: type === 'category' ? val('discountCategoryTarget') : (type === 'product' ? val('discountProductTarget') : 'all'),
      scope,
      size: val('discountSizeTarget'),
      fabric: val('discountFabricTarget'),
      color: val('discountColorTarget'),
      active: val('discountStatus') !== 'inactive',
      publishPages: selectedPublishPages()
    };
  }

  function ruleSame(r, s){
    return String(r.scope || '') === String(s.scope || '') &&
      String(r.size || '') === String(s.size || '') &&
      String(r.fabric || '') === String(s.fabric || '') &&
      String(r.color || '') === String(s.color || '');
  }

  function productMatchesSelection(p, s){
    if(s.type === 'all') return true;
    if(s.type === 'category') return String(p.category || '') === String(s.target || '');
    if(s.type === 'product') return String(p.id || '') === String(s.target || '') || String(p._dbId || '') === String(s.target || '');
    return false;
  }

  function validateSelection(percent, s){
    if(!s.type) return 'Please choose a discount target.';
    if(s.type !== 'all' && !s.target) return 'Please choose a discount target.';
    if(percent < 0 || percent > 90) return 'Discount must be between 0 and 90.';
    if(s.active && percent <= 0) return 'Please add discount percentage.';
    if(s.type === 'product' && ['size','combo','combo_color'].includes(s.scope) && !s.size) return 'Please choose a size for this product.';
    if(s.type === 'product' && ['fabric','combo','combo_color'].includes(s.scope) && !s.fabric) return 'Please choose a fabric for this product.';
    if(s.type === 'product' && ['color','combo_color'].includes(s.scope) && !s.color) return 'Please choose a color for this product.';
    return '';
  }

  async function persistProduct(p){
    if(!p._dbId) throw new Error('Product database ID is missing. Refresh admin and try again.');
    await adminApi('/products/' + encodeURIComponent(p._dbId), {method:'PUT', body:productPayload(p)});
  }

  window.applyDiscount = async function(){
    if(typeof window.hasAdminPermission === 'function' && !window.hasAdminPermission('discounts','write')) return show('You do not have write access for discounts.', true);
    const percent = num('bulkDiscount', 0);
    const s = getSelection();
    const validation = validateSelection(percent, s);
    if(validation) return show(validation, true);
    try{
      const products = await loadProducts();
      const pct = s.active ? Number(percent || 0) : 0;
      const changed = [];
      products.forEach(p => {
        if(!productMatchesSelection(p, s)) return;
        if(s.type === 'product' && s.scope !== 'product'){
          const rules = Array.isArray(p.discountRules) ? p.discountRules.filter(r => !ruleSame(r, s)) : [];
          if(s.active && pct > 0){
            rules.push({
              scope:s.scope,
              size:['size','combo','combo_color'].includes(s.scope) ? s.size : '',
              fabric:['fabric','combo','combo_color'].includes(s.scope) ? s.fabric : '',
              color:['color','combo_color'].includes(s.scope) ? s.color : '',
              percent:pct,
              active:true,
              updatedAt:new Date().toISOString(),
              publishPages:s.publishPages || ['discounted-items.html']
            });
          }
          p.discountRules = rules;
        }else{
          p.discountPercent = pct;
          p.discountPages = s.publishPages || ['discounted-items.html'];
        }
        changed.push(p);
      });
      if(!changed.length) return show('No matching product found for this discount.', true);
      for(const p of changed) await persistProduct(p);
      try{ window.products = products; localStorage.setItem('cvPrototypeProducts', JSON.stringify(products)); localStorage.setItem('adminProducts', JSON.stringify(products)); sessionStorage.setItem('cvPrototypeProducts', JSON.stringify(products)); sessionStorage.setItem('adminProducts', JSON.stringify(products)); }catch(e){}
      editKey = null;
      if(q('applyDiscountBtn')) q('applyDiscountBtn').textContent = 'Apply Discount';
      if(typeof window.renderProductsAdmin === 'function') window.renderProductsAdmin();
      await window.renderDiscountList();
      show('Discount saved and published to ' + changed.length + ' product(s).');
    }catch(e){ console.error('Apply discount failed', e); show('Could not save discount: ' + e.message, true); }
  };

  function describeRule(r){
    const bits = [];
    if(r.size) bits.push('Size: ' + r.size);
    if(r.fabric) bits.push('Fabric: ' + r.fabric);
    if(r.color) bits.push('Color: ' + r.color);
    const scopeLabel = String(r.scope || 'variant').replace('combo_color','Size + Fabric + Color').replace('combo','Size + Fabric').toUpperCase();
    return `${scopeLabel} discount: ${Number(r.percent || 0)}%${bits.length ? ' | ' + bits.join(' | ') : ''}${r.active === false ? ' | Inactive' : ''}`;
  }
  ...
  function entries(products){
    ...
    products.forEach(p => {
      ...
      if(Number(p.discountPercent || 0) > 0) out.push({key:'product:'+id, product:p, kind:'product', label:'Product discount: ' + Number(p.discountPercent || 0) + '%', percent:Number(p.discountPercent||0), scope:'product'});
      (Array.isArray(p.discountRules) ? p.discountRules : []).forEach((r, idx) => {
        if(!r || Number(r.percent || 0) <= 0) return;
        out.push({key:'rule:'+id+':'+idx, product:p, kind:'rule', ruleIndex:idx, label:describeRule(r), percent:Number(r.percent||0), scope:r.scope || '', size:r.size || '', fabric:r.fabric || '', color:r.color || '', active:r.active !== false});
      });
    });
    return out;
  }
  ...
  window.renderDiscountList = async function(){
    ...
    try{
      ...
      box.innerHTML = list.length ? list.map(item => `<div class="admin-item"><div><strong>${esc(item.product.name || item.product.id)}</strong><br>${esc(item.label)}</div><div style="display:flex;gap:8px;align-items:center;"><button type="button" data-discount-edit="${esc(item.key)}">Edit</button><button type="button" data-discount-delete="${esc(item.key)}">Delete</button></div></div>`).join('') : '<p>No active product discounts.</p>';
    }catch(e){ console.error('Render discount list failed', e); box.innerHTML = '<p>Could not load product discounts.</p>'; }
  };

  async function findEntry(key){ return entries(await loadProducts()).find(x => String(x.key) === String(key)); }

  window.editDiscountOnPage = async function(key){
    try{
      const item = await findEntry(key);
      if(!item) return show('This discount could not be found. Refresh and try again.', true);
      if(q('discountTargetType')) q('discountTargetType').value = 'product';
      await window.renderDiscountTargets();
      if(q('discountProductTarget')) q('discountProductTarget').value = item.product.id || item.product._dbId || '';
      if(q('discountApplyScope')) q('discountApplyScope').value = item.scope || 'product';
      await window.renderDiscountVariantTargets();
      if(q('discountSizeTarget')) q('discountSizeTarget').value = item.size || '';
      if(q('discountFabricTarget')) q('discountFabricTarget').value = item.fabric || '';
      if(q('discountColorTarget')) q('discountColorTarget').value = item.color || '';
      if(q('bulkDiscount')) q('bulkDiscount').value = item.percent || '';
      if(q('discountStatus')) q('discountStatus').value = item.active === false ? 'inactive' : 'active';
      editKey = key;
      if(q('applyDiscountBtn')) q('applyDiscountBtn').textContent = 'Update Discount';
      show('Discount loaded for editing on this page.');
    }catch(e){ console.error('Edit discount failed', e); show('Could not load discount for editing: ' + e.message, true); }
  };
  ...
  window.deleteDiscountOnPage = async function(key){
    ...
    try{
      ...
      for(const p of products){
        const id = p.id || p._dbId || '';
        if(String(key) === 'product:' + id){ p.discountPercent = 0; changed = p; break; }
        const m = String(key).match(/^rule:(.*):(\d+)$/);
        if(m && String(m[1]) === String(id) && Array.isArray(p.discountRules)){
          const idx = Number(m[2]);
          if(p.discountRules[idx]){ p.discountRules.splice(idx, 1); changed = p; break; }
        }
      }
      if(!changed) return show('This discount could not be found. Refresh and try again.', true);
      await persistProduct(changed);
      await window.renderDiscountList();
      show('Discount deleted.');
    }catch(e){ console.error('Delete discount failed', e); show('Could not delete discount: ' + e.message, true); }
  };

  document.addEventListener('change', function(e){
    if(!e.target) return;
    if(e.target.id === 'discountTargetType') window.renderDiscountTargets();
    if(['discountProductTarget','discountApplyScope'].includes(e.target.id)) window.renderDiscountVariantTargets();
  }, true);

  document.addEventListener('click', function(e){
    const rawEl = e.target.closest && e.target.closest('[onclick]');
    if(rawEl){
      const raw = rawEl.getAttribute('onclick') || '';
      const fn = (raw.match(/^([A-Za-z_$][\w$]*)\(/) || [])[1];
      if(['applyDiscount','saveDiscountPage','clearDiscounts','renderDiscountTargets'].includes(fn)){
        e.preventDefault(); e.stopPropagation();
        if(typeof window[fn] === 'function') window[fn]();
        return;
      }
    }
    const edit = e.target.closest && e.target.closest('[data-discount-edit]');
    if(edit){ e.preventDefault(); e.stopPropagation(); window.editDiscountOnPage(edit.getAttribute('data-discount-edit')); return; }
    const del = e.target.closest && e.target.closest('[data-discount-delete]');
    if(del){ e.preventDefault(); e.stopPropagation(); window.deleteDiscountOnPage(del.getAttribute('data-discount-delete')); return; }
  }, true);
