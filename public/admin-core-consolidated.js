...
function describeDiscountRule(r){
  ...
  if(r.applyScope === 'combo_color') parts.push('Size: ' + r.size + ' + Fabric: ' + r.fabric + ' + Color: ' + r.color);
...
(function(){
  ...
  function getProductColorsForDiscount(p){
    ...
    const add = (v) => {
      if(v === undefined || v === null) return;
      if(typeof v === 'object') v = v.label || v.name || v.value || v.color || v.colour || v.title || '';
      String(v).split(',').forEach(x => { const t = x.trim(); if(t) out.push(t); });
    };
    ...
    if(p && Array.isArray(p.imageMeta)){
      p.imageMeta.forEach(item => add(item && (item.color || item.colorName || item.colour || item.label)));
    }
    if(p && Array.isArray(p.galleryMeta)){
      p.galleryMeta.forEach(item => add(item && (item.color || item.colorName || item.colour || item.label)));
    }
    return [...new Set(out.map(x=>String(x || '').trim()).filter(Boolean))];
  }
  ...
  function ensureDiscountApplyScopeOptions(){
    ...
    el.innerHTML = [
      ...
      '<option value="combo_color">Size + Fabric + Color Combination</option>'
    ].join('');
    el.value = ['product','size','fabric','color','combo','combo_color'].includes(current) ? current : 'product';
  }
  ...
  async function renderDiscountVariantTargets(){
    ...
    if(applyScope === 'size' || applyScope === 'combo' || applyScope === 'combo_color'){
    ...
    if(applyScope === 'fabric' || applyScope === 'combo' || applyScope === 'combo_color'){
    ...
    if(colorEl){
      if(applyScope === 'color' || applyScope === 'combo_color'){
  ...
  window.renderDiscountTargets = async function(){
    ...
    const selectedCategoryTarget = categoryEl.value;
    const selectedProductTarget = productEl.value;
    ...
    try{
      ...
      if(type === 'category'){
        ...
        categoryEl.innerHTML = cats.length ? cats.map(c=>`<option value="${esc(c)}" ${String(c) === String(selectedCategoryTarget) ? 'selected' : ''}>${esc(c)}</option>`).join('') : '<option value="">No categories found</option>';
      }

      if(type === 'product'){
        showDiscountSelect('discountProductTarget');
        productEl.innerHTML = products.length ? products.map(p=>`<option value="${esc(p.id)}" ${String(p.id) === String(selectedProductTarget) ? 'selected' : ''}>${esc(p.name || p.id)}</option>`).join('') : '<option value="">No products found</option>';
      ...
      if(selection.type === 'product' && selection.applyScope !== 'product'){
        ...
        if(selection.active && pct > 0){
          rules.push({
            ...
            fabric: selection.applyScope === 'fabric' || selection.applyScope === 'combo' || selection.applyScope === 'combo_color' ? selection.fabric : '',
            color: selection.applyScope === 'color' || selection.applyScope === 'combo_color' ? selection.color : '',
            percent: pct,
            active: true,
  ...
  window.applyDiscount = async function(){
    ...
    if(selection.type === 'product' && (selection.applyScope === 'size' || selection.applyScope === 'combo' || selection.applyScope === 'combo_color') && !selection.size) return show('Please choose a size for this product.', true);
    if(selection.type === 'product' && (selection.applyScope === 'fabric' || selection.applyScope === 'combo' || selection.applyScope === 'combo_color') && !selection.fabric) return show('Please choose a fabric for this product.', true);
    if(selection.type === 'product' && (selection.applyScope === 'color' || selection.applyScope === 'combo_color') && !selection.color) return show('Please choose a color for this product.', true);
