(function(){
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .cv-review-bottom-anchor{grid-column:1 / -1;width:100%;margin-top:18px;}
    `;
  ...
  function mountModalReviews(productId, force){
    ...
    // Keep reviews at the bottom of the modal and do not re-render while the
    // customer is selecting stars or typing a review. Re-rendering was causing
    // the form to reset every time the observer/interval fired.
    const stableBody = document.getElementById('cvStableQuickViewBody');
    if(stableBody){
      const duplicateTargets = Array.from(stableBody.querySelectorAll('#cvStableProductReviews'));
      let stableTarget = duplicateTargets.shift();
      duplicateTargets.forEach(node => node.remove());

      if(!stableTarget){
        stableTarget = document.createElement('div');
        stableTarget.id = 'cvStableProductReviews';
        stableTarget.className = 'cv-review-bottom-anchor';
        stableBody.appendChild(stableTarget);
      } else if(stableTarget.parentNode !== stableBody || stableTarget !== stableBody.lastElementChild){
        stableBody.appendChild(stableTarget);
      }

      if(!force && stableTarget.dataset.reviewProductId === productId && stableTarget.querySelector('[data-cv-review-panel]')){
        return;
      }

      stableTarget.dataset.reviewProductId = productId;
  ...
  async function submitReview(productId){
    ...
    mountModalReviews(productId, true);
  ...
  function wrapOpeners(){
    ['openProduct','openCustomizeProduct'].forEach(name => {
      ...
      const wrapped = function(id){
        ...
        setTimeout(() => mountModalReviews(state.currentProductId, true), 120);
        return out;
      };
  ...
  document.addEventListener('click', function(e){
    ...
    if(open){
      ...
      setTimeout(() => mountModalReviews(productId, true), 150);
      return;
    }
  ...
  function init(){
    ...
    setInterval(function(){
      ...
      const body = document.getElementById('cvStableQuickViewBody');
      const productId = activeProductId();
      const target = body && body.querySelector('#cvStableProductReviews');
      if(stable && !stable.classList.contains('hidden') && productId && (!target || target.dataset.reviewProductId !== productId)){
        mountModalReviews(productId, true);
      }
    }, 1200);
  }
