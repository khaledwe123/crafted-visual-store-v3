...
/* CV DISCOUNTED ITEMS + VARIANT DISCOUNT PATCH 20260614
   Scope: shop/discounted-items frontend price logic only.
   Adds exact discount matching for Product, Size, Fabric, Color, Size+Fabric, and Size+Fabric+Color.
*/
(function(){
  if(window.__cvDiscountedItemsVariantPatch20260614) return;
  window.__cvDiscountedItemsVariantPatch20260614 = true;

  function dNorm(v){ return String(v == null ? '' : v).trim().toLowerCase(); }
  function dNum(v){ var n = Number(v || 0); return Number.isFinite(n) ? n : 0; }
  function dMoney(v){ try{ return typeof money === 'function' ? money(v) : ('SAR ' + Math.round(Number(v||0)).toLocaleString()); }catch(e){ return 'SAR ' + Math.round(Number(v||0)).toLocaleString(); } }
  function dVat(p){ try{ return typeof vatRate === 'function' ? vatRate(p) : dNum(p && p.vatRate) || 15; }catch(e){ return dNum(p && p.vatRate) || 15; } }
  function dSizeLabel(sizeOpt){ return String((sizeOpt && (sizeOpt.label || sizeOpt.name || sizeOpt.size || sizeOpt.value)) || sizeOpt || '').trim(); }
  function dFabricLabel(fabricOpt){ return String((fabricOpt && (fabricOpt.label || fabricOpt.name || fabricOpt.fabric || fabricOpt.value)) || fabricOpt || '').trim(); }
  function dColorLabel(color){
    try{
      if(color) return String(color).trim();
      if(typeof selectedColor !== 'undefined' && selectedColor) return String(selectedColor).trim();
      if(window.selectedColor) return String(window.selectedColor).trim();
    }catch(e){}
    return '';
  }
  function dProductColors(p){
    var keys = [];
    if(p && p.colors && typeof p.colors === 'object') keys = Object.keys(p.colors);
    if(!keys.length && Array.isArray(p && p.colorOptions)) keys = p.colorOptions.map(function(c){ return c && (c.label || c.name || c.color || c.value) || c; });
    if(!keys.length) keys = [''];
    return keys.map(function(x){ return String(x || '').trim(); }).filter(function(x,i,a){ return a.indexOf(x) === i; });
  }
  function dRuleScope(r){ return String(r && (r.scope || r.applyScope || r.type || r.targetScope) || 'product').trim(); }
  function dRuleActive(r){
    if(!r) return false;
    if(r.active === false || r.enabled === false || r.status === 'inactive') return false;
    var pct = dNum(r.percent || r.discountPercent || r.discount_percent || r.value);
  ...
  function dRuleMatchesVariant(r, p, sizeOpt, fabricOpt, color){
    if(!dRuleActive(r)) return false;
    var scope = dRuleScope(r);
    var size = dSizeLabel(sizeOpt);
    var fabric = dFabricLabel(fabricOpt);
    var clr = dColorLabel(color);
    var rSize = String(r.size || r.sizeLabel || r.variantSize || '').trim();
    var rFabric = String(r.fabric || r.fabricLabel || r.variantFabric || '').trim();
    var rColor = String(r.color || r.colour || r.colorLabel || r.variantColor || '').trim();

    if(scope === 'product') return true;
    if(scope === 'size') return !!rSize && dNorm(rSize) === dNorm(size);
    if(scope === 'fabric') return !!rFabric && dNorm(rFabric) === dNorm(fabric);
    if(scope === 'color') return !!rColor && dNorm(rColor) === dNorm(clr);
    if(scope === 'combo' || scope === 'size_fabric'){
      if(rColor) return dNorm(rSize) === dNorm(size) && dNorm(rFabric) === dNorm(fabric) && dNorm(rColor) === dNorm(clr);
      return dNorm(rSize) === dNorm(size) && dNorm(rFabric) === dNorm(fabric);
    }
    if(scope === 'combo_color' || scope === 'variant' || scope === 'size_fabric_color'){
      return dNorm(rSize) === dNorm(size) && dNorm(rFabric) === dNorm(fabric) && dNorm(rColor) === dNorm(clr);
    }
    // Backward compatibility: if a saved rule has dimensions but no recognized scope, infer the strictest match.
    if(rSize || rFabric || rColor){
      if(rSize && dNorm(rSize) !== dNorm(size)) return false;
      if(rFabric && dNorm(rFabric) !== dNorm(fabric)) return false;
      if(rColor && dNorm(rColor) !== dNorm(clr)) return false;
      return true;
    }
    return false;
  }
  function dVariantDiscountPercent(p, sizeOpt, fabricOpt, color){
    p = p || {};
    var best = dNum(p.discountPercent || p.discount_percent || p.discount);
    var rules = Array.isArray(p.discountRules) ? p.discountRules : [];
    rules.forEach(function(r){
      if(dRuleMatchesVariant(r, p, sizeOpt, fabricOpt, color)){
        best = Math.max(best, dNum(r.percent || r.discountPercent || r.discount_percent || r.value));
      }
    });
    return Math.max(0, Math.min(90, best));
  }
  function dProductHasDiscount(p){
    if(dNum(p && (p.discountPercent || p.discount_percent || p.discount)) > 0) return true;
    return (Array.isArray(p && p.discountRules) ? p.discountRules : []).some(dRuleActive);
  }
  window.cvVariantDiscountPercent = dVariantDiscountPercent;
  window.cvProductHasDiscount = dProductHasDiscount;

  var prevFinalPrice = typeof window.finalPrice === 'function' ? window.finalPrice : (typeof finalPrice === 'function' ? finalPrice : null);
  window.finalPrice = finalPrice = function(p, sizeOpt, fabricOpt){
    var incl = 0;
    try{ incl = typeof priceIncludingVat === 'function' ? priceIncludingVat(p, sizeOpt, fabricOpt) : dNum(p && p.price) * (1 + dVat(p)/100); }
    catch(e){ incl = dNum(p && p.price) * (1 + dVat(p)/100); }
    var pct = dVariantDiscountPercent(p, sizeOpt, fabricOpt, dColorLabel());
    return Math.round(incl * (1 - pct / 100));
  };

  function dPriceBefore(p, s, f){ try{ return typeof priceBeforeVat === 'function' ? priceBeforeVat(p,s,f) : dNum(p && p.price); }catch(e){ return dNum(p && p.price); } }
  function dIncl(p,s,f){ try{ return typeof priceIncludingVat === 'function' ? priceIncludingVat(p,s,f) : dPriceBefore(p,s,f) * (1 + dVat(p)/100); }catch(e){ return dPriceBefore(p,s,f) * (1 + dVat(p)/100); } }

  function dAllVariantPrices(p){
    try{ if(typeof normalizeProduct === 'function') p = normalizeProduct(p || {}); }catch(e){}
    var sizes = Array.isArray(p && p.sizeOptions) && p.sizeOptions.length ? p.sizeOptions : [null];
    var fabrics = Array.isArray(p && p.fabricOptions) && p.fabricOptions.length ? p.fabricOptions : [null];
    var colors = dProductColors(p);
    var values = [];
    sizes.forEach(function(s){ fabrics.forEach(function(f){ colors.forEach(function(c){
      var before = dPriceBefore(p,s,f);
      if(before > 0){
        var incl = dIncl(p,s,f);
        var pct = dVariantDiscountPercent(p,s,f,c);
        values.push({before:before, incl:incl, final:Math.round(incl * (1 - pct/100)), discount:pct, size:s, fabric:f, color:c});
      }
    }); }); });
    if(!values.length){
      var b = dNum(p && p.price), i = b * (1 + dVat(p)/100), pct = dVariantDiscountPercent(p,null,null,'');
      values.push({before:b, incl:i, final:Math.round(i*(1-pct/100)), discount:pct, size:null, fabric:null, color:''});
    }
    return values;
  }
  function dBestDisplayVariant(p){
    var vals = dAllVariantPrices(p);
    var discounted = vals.filter(function(x){ return x.discount > 0; }).sort(function(a,b){ return a.final-b.final; });
    return discounted[0] || vals.sort(function(a,b){ return a.final-b.final; })[0];
  }

  window.shopCardPriceHTML = shopCardPriceHTML = function(p){
    var low = dBestDisplayVariant(p || {});
    if(low && low.discount > 0){
      return '<div class="shop-price-summary">' +
        '<small>From</small><br>' +
        '<span class="old-price" style="text-decoration:line-through;opacity:.65;">' + dMoney(Math.round(low.incl)) + '</span><br>' +
        '<strong class="discount-price">' + dMoney(low.final) + '</strong> ' +
        '<span class="discount-badge">-' + low.discount + '%</span>' +
      '</div>';
    }
    return '<div class="shop-price-summary"><small>From</small><br><strong>' + dMoney(low ? low.final : 0) + '</strong></div>';
  };

  window.priceHTML = priceHTML = function(p, sizeOpt, fabricOpt){
    try{ if(typeof normalizeProduct === 'function') p = normalizeProduct(p || {}); }catch(e){}
    var before = dPriceBefore(p, sizeOpt, fabricOpt);
    var vat = before * dVat(p) / 100;
    var incl = before + vat;
    var pct = dVariantDiscountPercent(p, sizeOpt, fabricOpt, dColorLabel());
    var fp = Math.round(incl * (1 - pct / 100));
    var html = '<div class="price-detail">' +
      '<div>Selected price before VAT: <strong>' + dMoney(before) + '</strong></div>' +
      '<div>VAT (' + dVat(p) + '%): <strong>' + dMoney(vat) + '</strong></div>';
    if(pct > 0){
      html += '<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">' + dMoney(incl) + '</strong></div>' +
        '<div>After discount: <strong class="discount-price">' + dMoney(fp) + '</strong> <span class="discount-badge">-' + pct + '% OFF</span></div>';
    }else{
      html += '<div>Total incl. VAT: <strong>' + dMoney(incl) + '</strong></div>';
    }
    return html + '</div>';
  };

  var prevApplySort = typeof window.applySortAndFilter === 'function' ? window.applySortAndFilter : null;
  window.applySortAndFilter = applySortAndFilter = function(){
    try{
      if(Array.isArray(window.products)) products = window.products;
      products = (products || []).map(function(p){ try{ return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p; }catch(e){ return p; } });
      var list = products.slice();
      var catFilter = document.getElementById('categoryFilter');
      var sortSelect = document.getElementById('sortSelect');
      var selectedCategory = catFilter && catFilter.value ? catFilter.value : (window.CV_CATEGORY_FILTER || (typeof activeCategory !== 'undefined' ? activeCategory : 'All'));
      try{ activeCategory = selectedCategory || 'All'; }catch(e){}
      if(window.CV_DISCOUNTED_ONLY) list = list.filter(dProductHasDiscount);
      if(selectedCategory && selectedCategory !== 'All') list = list.filter(function(p){ return String(p.category || '') === String(selectedCategory); });
      var sort = sortSelect ? sortSelect.value : 'featured';
      if(sort === 'priceHigh') list.sort(function(a,b){ return dBestDisplayVariant(b).final - dBestDisplayVariant(a).final; });
      else if(sort === 'priceLow') list.sort(function(a,b){ return dBestDisplayVariant(a).final - dBestDisplayVariant(b).final; });
      else if(sort === 'discount') list.sort(function(a,b){ return (dBestDisplayVariant(b).discount || 0) - (dBestDisplayVariant(a).discount || 0); });
      else if(sort === 'rating' && typeof productRating === 'function') list.sort(function(a,b){ return productRating(b.id).avg-productRating(a.id).avg; });
      ...
      console.warn('Discounted/variant filter failed; using previous shop renderer.', e);
      if(prevApplySort) return prevApplySort.apply(this, arguments);
    }
  };

  function refreshDiscountedPrice(){
    try{
      var el = document.getElementById('modalPrice');
      if(el && (typeof currentProduct !== 'undefined') && currentProduct){
        el.innerHTML = priceHTML(currentProduct, typeof selectedSizeOption !== 'undefined' ? selectedSizeOption : null, typeof selectedFabricOption !== 'undefined' ? selectedFabricOption : null);
      }
    }catch(e){}
  }
  ['updateSelectedFabric','updateSelectedSize','selectColor'].forEach(function(name){
    var old = window[name];
    if(typeof old === 'function'){
      window[name] = function(){ var r = old.apply(this, arguments); setTimeout(refreshDiscountedPrice, 10); return r; };
      try{ if(name === 'updateSelectedFabric') updateSelectedFabric = window[name]; if(name === 'updateSelectedSize') updateSelectedSize = window[name]; if(name === 'selectColor') selectColor = window[name]; }catch(e){}
    }
  });

  var oldAdd = typeof window.addCurrentToCart === 'function' ? window.addCurrentToCart : null;
  if(oldAdd){
    window.addCurrentToCart = addCurrentToCart = function(){
      var beforeLen = (window.cart || cart || []).length;
      var result = oldAdd.apply(this, arguments);
      try{
        var list = window.cart || cart || [];
        if(list.length > beforeLen){
          var item = list[list.length - 1];
          var pct = dVariantDiscountPercent(currentProduct, selectedSizeOption, selectedFabricOption, dColorLabel());
          item.discountPercent = pct;
          item.price = finalPrice(currentProduct, selectedSizeOption, selectedFabricOption);
          if(typeof saveCartSafe === 'function') saveCartSafe();
  ...
  function rerenderDiscountPatch(){ try{ if(document.getElementById('productGrid')) applySortAndFilter(); }catch(e){} }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(rerenderDiscountPatch, 400); });
  else setTimeout(rerenderDiscountPatch, 400);
})();
