...
/* CV DISCOUNT DISPLAY PATCH - Size + Fabric + Color exact variant display
   Scope: shop/frontend price display only. Does not modify admin, cart storage, auth, menu, page builder, analytics, or styling. */
(function(){
  'use strict';
  if(window.__cvDiscountVariantColorDisplayPatch) return;
  window.__cvDiscountVariantColorDisplayPatch = true;

  function n(v){ return Number(v || 0) || 0; }
  function clean(v){ return String(v == null ? '' : v).trim(); }
  function key(v){ return clean(v).toLowerCase().replace(/\s+/g,' ').replace(/[×x]/g,'x'); }
  function moneySafe(v){
    try{ if(typeof money === 'function') return money(v); }catch(e){}
    return 'SAR ' + Math.round(n(v)).toLocaleString();
  }
  function labelOf(v){ return clean(v && typeof v === 'object' ? (v.label || v.name || v.title || v.value) : v); }
  function currentColorFallback(){
    try{ if(window.selectedColor) return window.selectedColor; }catch(e){}
    try{ if(typeof selectedColor !== 'undefined' && selectedColor) return selectedColor; }catch(e){}
    return '';
  }
  function productId(p){ return clean(p && (p.id || p.productId || p._dbId || p.sku || p.code)); }
  function rulePercent(r){ return n(r && (r.percent || r.discountPercent || r.discount || r.value)); }
  function ruleActive(r){
    if(!r) return false;
    if(r.active === false || r.enabled === false) return false;
    if(String(r.status || '').toLowerCase() === 'inactive') return false;
    const now = Date.now();
    const start = r.startDate || r.start_date || r.startsAt || r.starts_at;
    const end = r.endDate || r.end_date || r.expiresAt || r.expires_at;
    if(start && !isNaN(Date.parse(start)) && Date.parse(start) > now) return false;
    if(end && !isNaN(Date.parse(end)) && Date.parse(end) < now) return false;
    return rulePercent(r) > 0;
  }
  function ruleProductMatches(r, p){
    const target = key(r.targetType || r.type || r.target || 'product');
    if(target === 'all' || target === 'all products') return true;
    if(target === 'category' || target === 'product category') return key(p.category || p.category_en || p.categoryName) === key(r.category || r.categoryName);
    return key(r.productId || r.product_id || r.product || r.idTarget || '') === key(productId(p)) || !clean(r.productId || r.product_id || r.product || r.idTarget || '');
  }
  function ruleVariantMatches(r, p, sizeOpt, fabricOpt, colorName){
    if(!ruleActive(r) || !ruleProductMatches(r, p)) return false;
    const scope = key(r.applyScope || r.scope || r.discountScope || 'product');
    const size = labelOf(sizeOpt);
    const fabric = labelOf(fabricOpt);
    const color = clean(colorName || currentColorFallback());
    const rSize = clean(r.size || r.sizeLabel || r.size_name || r.variantSize);
    const rFabric = clean(r.fabric || r.fabricLabel || r.fabric_name || r.variantFabric);
    const rColor = clean(r.color || r.colour || r.colorName || r.colorLabel || r.variantColor);
    const needsSize = scope.includes('size') || scope.includes('combo') || !!rSize;
    const needsFabric = scope.includes('fabric') || scope.includes('combo') || !!rFabric;
    const needsColor = scope.includes('color') || scope.includes('colour') || !!rColor;
    if(needsSize && rSize && key(rSize) !== key(size)) return false;
    if(needsFabric && rFabric && key(rFabric) !== key(fabric)) return false;
    if(needsColor && rColor && key(rColor) !== key(color)) return false;
    if(needsSize && !rSize && !size) return false;
    if(needsFabric && !rFabric && !fabric) return false;
    if(needsColor && !rColor && !color) return false;
    return true;
  }
  function rulesFor(p){ return Array.isArray(p && p.discountRules) ? p.discountRules : []; }
  function discountForVariant(p, sizeOpt, fabricOpt, colorName){
    p = p || {};
    let best = n(p.discountPercent);
    rulesFor(p).forEach(function(r){
      if(ruleVariantMatches(r, p, sizeOpt, fabricOpt, colorName)) best = Math.max(best, rulePercent(r));
    });
    return best;
  }
  function beforeVatSafe(p, sizeOpt, fabricOpt){
    try{ if(typeof priceBeforeVat === 'function') return n(priceBeforeVat(p, sizeOpt, fabricOpt)); }catch(e){}
    const sLabel = labelOf(sizeOpt);
    if(fabricOpt && fabricOpt.sizePrices && fabricOpt.sizePrices[sLabel] !== undefined) return n(fabricOpt.sizePrices[sLabel]);
    if(sizeOpt && sizeOpt.price !== undefined) return n(sizeOpt.price);
    return n(p && p.price);
  }
  function vatRateSafe(p){
    try{ if(typeof vatRate === 'function') return n(vatRate(p)); }catch(e){}
    try{ return n(p.vatRate || settings.vat_rate || 15); }catch(e){ return n(p && p.vatRate) || 15; }
  }
  function inclVatSafe(p, sizeOpt, fabricOpt){ return beforeVatSafe(p, sizeOpt, fabricOpt) * (1 + vatRateSafe(p) / 100); }

  window.cvDiscountForVariant = discountForVariant;

  const oldFinalPrice = window.finalPrice || (typeof finalPrice === 'function' ? finalPrice : null);
  window.finalPrice = function(p, sizeOpt, fabricOpt, colorName){
    const incl = inclVatSafe(p, sizeOpt, fabricOpt);
    const disc = discountForVariant(p, sizeOpt, fabricOpt, colorName || currentColorFallback());
    return Math.round(incl * (1 - disc / 100));
  };
  try{ finalPrice = window.finalPrice; }catch(e){}

  window.priceHTML = function(p, sizeOpt, fabricOpt, colorName){
    p = p || {};
    const before = beforeVatSafe(p, sizeOpt, fabricOpt);
    const vatRateNum = vatRateSafe(p);
    const vat = before * vatRateNum / 100;
    const incl = before + vat;
    const activeColor = colorName || currentColorFallback();
    const disc = discountForVariant(p, sizeOpt, fabricOpt, activeColor);
    const fp = Math.round(incl * (1 - disc / 100));
    let html = '<div class="price-detail">' +
      '<div>Selected price before VAT: <strong>' + moneySafe(before) + '</strong></div>' +
      '<div>VAT (' + vatRateNum + '%): <strong>' + moneySafe(vat) + '</strong></div>';
    if(disc > 0){
      html += '<div>Total before discount: <strong class="old-price" style="text-decoration:line-through;opacity:.65;">' + moneySafe(incl) + '</strong></div>' +
        '<div>After discount: <strong class="discount-price" style="color:#b11226;">' + moneySafe(fp) + '</strong> <span class="discount-badge" style="color:#fff;background:#d71920;border-radius:999px;padding:3px 8px;font-weight:800;">-' + disc + '% OFF</span></div>';
    }else{
      html += '<div>Total incl. VAT: <strong>' + moneySafe(incl) + '</strong></div>';
    }
    html += '</div>';
    return html;
  };
  try{ priceHTML = window.priceHTML; }catch(e){}

  function selectOptions(p){
    try{ p = typeof normalizeProduct === 'function' ? normalizeProduct(p || {}) : (p || {}); }catch(e){ p = p || {}; }
    const sizes = Array.isArray(p.sizeOptions) && p.sizeOptions.length ? p.sizeOptions : [{label:'Custom Size', price:n(p.price)}];
    const fabrics = Array.isArray(p.fabricOptions) && p.fabricOptions.length ? p.fabricOptions : [{label:'Standard Fabric', sizePrices:{}}];
    const colors = Object.keys(p.colors || {});
    return {sizes:sizes, fabrics:fabrics, colors:colors.length ? colors : ['']};
  }
  window.cvHasAnyActiveDiscount = function(p){
    if(n(p && p.discountPercent) > 0) return true;
    return rulesFor(p).some(ruleActive);
  };
  window.cvLowestDiscountedVariant = function(p){
    const opts = selectOptions(p);
    let best = null;
    opts.sizes.forEach(function(s){ opts.fabrics.forEach(function(f){ opts.colors.forEach(function(c){
      const before = inclVatSafe(p,s,f);
      const disc = discountForVariant(p,s,f,c);
      const final = Math.round(before * (1 - disc / 100));
      const item = {before:before, final:final, discount:disc, size:s, fabric:f, color:c};
      if(!best || (disc > 0 && final < best.final) || (!best.discount && final < best.final)) best = item;
    }); }); });
    return best || {before:inclVatSafe(p), final:Math.round(inclVatSafe(p)), discount:0};
  };

  // Refresh stable quick view price after color changes, because variant discounts can be color-specific.
  document.addEventListener('click', function(e){
    const btn = e.target && e.target.closest ? e.target.closest('[data-cv-qv-color], .color-chip, [data-color], [data-product-color]') : null;
    if(!btn) return;
    setTimeout(function(){
      const box = document.getElementById('cvStableQvPrice') || document.getElementById('modalPrice');
      let p = null, s = null, f = null, c = '';
      try{ p = window.currentProduct || currentProduct; }catch(err){}
      try{ s = window.selectedSizeOption || selectedSizeOption; }catch(err){}
      try{ f = window.selectedFabricOption || selectedFabricOption; }catch(err){}
      try{ c = window.selectedColor || selectedColor; }catch(err){}
      if(box && p) box.innerHTML = window.priceHTML(p, s, f, c);
    }, 30);
  }, true);
})();
