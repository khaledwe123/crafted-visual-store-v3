...
async function loadAdminUsersFromBackend(){
  if(!((window.CV_API || (typeof CV_API !== "undefined" && CV_API)) && (window.CV_API || CV_API).token(true))) return [];
  try{
...
async function addAdminUser(){
  ...
  try{
    if(!(typeof CV_API !== "undefined" || window.CV_API)) throw new Error("API helper is not loaded.");
    const apiHelper = (window.CV_API || CV_API);
    await apiHelper.request('/admin-users', {method:'POST', admin:true, body:{name,email,password,role,permissions,active:true}});
