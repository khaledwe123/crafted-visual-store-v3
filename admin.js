...
function addDiscountCode(){
  ...
  const expiry = document.getElementById("discountCodeExpiry").value;
  ...
  renderDiscountCodeList();
  showAdminStatus("Discount code created.");
}
function toggleDiscountCode(i){
  if(typeof hasAdminPermission === "function" && !hasAdminPermission("discounts","write")){ showAdminStatus("You have read-only access for this section.", true); return; }
...
function renderDiscountCodeList(){
  ...
  if(!box) return;
  const codes = getDiscountCodes();
  box.innerHTML = codes.length ? codes.map((c,i)=>`
    <div class="admin-item">
      <div><strong>${c.code}</strong><br>${c.percent}% discount ${c.expiry ? "| Expiry: "+c.expiry : ""}<br>Status: ${c.active ? "Active" : "Inactive"}</div>
      <div><button type="button" onclick="toggleDiscountCode(${i})">${c.active ? "Deactivate" : "Activate"}</button><button type="button" onclick="deleteDiscountCode(${i})">Delete</button></div>
    </div>
