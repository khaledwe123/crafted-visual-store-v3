...
/* === Discount page only final fix: target selection + product/size/fabric loading === */
function cvDiscountEsc(value){
...
function cvDiscountCategoryName(c){
  ...
  return c.label_en || c.name_en || c.name || c.category || '';
}
function cvDiscountProductLabel(p){
  return (p && (p.name || p.name_en || p.title || p.id || p.sku)) || '';
}
function cvDiscountProductId(p){
  return String((p && (p.id || p.sku || p._dbId)) || '');
}
function cvDiscountSizeLabels(p){
  const out = [];
  (p?.sizeOptions || []).forEach(function(s){ out.push(typeof s === 'string' ? s : (s.label || s.name || s.size)); });
  (p?.sizes || []).forEach(function(s){ out.push(typeof s === 'string' ? s : (s.label || s.name || s.size)); });
  return Array.from(new Set(out.filter(Boolean)));
}
function cvDiscountFabricLabels(p){
  const out = [];
  (p?.fabricOptions || []).forEach(function(f){ out.push(typeof f === 'string' ? f : (f.label || f.name || f.fabric)); });
  (p?.fabrics || []).forEach(function(f){ out.push(typeof f === 'string' ? f : (f.label || f.name || f.fabric)); });
  return Array.from(new Set(out.filter(Boolean)));
}
async function cvDiscountReloadProducts(){
  try{
    if(typeof loadProductsDataAdmin === 'function'){
      const fresh = await loadProductsDataAdmin();
      if(Array.isArray(fresh) && fresh.length){
        products = fresh.map(function(p){ try{return typeof normalizeProduct === 'function' ? normalizeProduct(p) : p;}catch(e){return p;} });
        try{ prototypeProductsWrite(products); }catch(e){}
      }
    }
  }catch(e){ console.warn('Discount product reload failed:', e); }
  return Array.isArray(products) ? products : [];
}
async function renderDiscountTargets(){
  await cvDiscountReloadProducts();
  const type = document.getElementById('discountTargetType')?.value || 'all';
  const categorySelect = document.getElementById('discountCategoryTarget');
  const productSelect = document.getElementById('discountProductTarget');
  const productOptionsBox = document.getElementById('discountSpecificProductOptions');
  if(categorySelect){
    const catList = (Array.isArray(categories) && categories.length ? categories : DEFAULT_CATEGORIES).map(cvDiscountCategoryName).filter(Boolean);
    categorySelect.innerHTML = catList.length
      ? catList.map(function(c){ return '<option value="'+cvDiscountEsc(c)+'">'+cvDiscountEsc(c)+'</option>'; }).join('')
      : '<option value="">No categories found</option>';
    categorySelect.style.display = type === 'category' ? '' : 'none';
  }
  if(productSelect){
    const list = (products || []).filter(function(p){ return p && cvDiscountProductId(p) && cvDiscountProductLabel(p); });
    productSelect.innerHTML = list.length
      ? '<option value="">Choose product</option>' + list.map(function(p){ return '<option value="'+cvDiscountEsc(cvDiscountProductId(p))+'">'+cvDiscountEsc(cvDiscountProductLabel(p))+'</option>'; }).join('')
      : '<option value="">No products found - refresh products first</option>';
    productSelect.style.display = type === 'product' ? '' : 'none';
  }
  if(productOptionsBox) productOptionsBox.style.display = type === 'product' ? '' : 'none';
...
function renderDiscountProductOptions(){
  ...
  const p = (products || []).find(function(x){ return cvDiscountProductId(x) === String(productId); });
  ...
  const showSize = type === 'product' && (scope === 'size' || scope === 'size_fabric');
  const showFabric = type === 'product' && (scope === 'fabric' || scope === 'size_fabric');
  if(sizeSelect){
    const sizes = p ? cvDiscountSizeLabels(p) : [];
    sizeSelect.innerHTML = sizes.length ? sizes.map(function(s){ return '<option value="'+cvDiscountEsc(s)+'">'+cvDiscountEsc(s)+'</option>'; }).join('') : '<option value="">No sizes for this product</option>';
    sizeSelect.style.display = showSize ? '' : 'none';
  }
  if(fabricSelect){
    const fabrics = p ? cvDiscountFabricLabels(p) : [];
    fabricSelect.innerHTML = fabrics.length ? fabrics.map(function(f){ return '<option value="'+cvDiscountEsc(f)+'">'+cvDiscountEsc(f)+'</option>'; }).join('') : '<option value="">No fabrics for this product</option>';
    fabricSelect.style.display = showFabric ? '' : 'none';
  }
}
function cvDiscountBuildProductPayload(p){
  return {
    sku: p.id || p.sku,
    ...
    description_ar: p.description_ar || '',
    base_price: Number(p.price || p.base_price || 0),
    vat_rate: Number(p.vatRate || p.vat_rate || 15),
    active: true,
    data: p
  };
}
async function cvDiscountSaveChangedProducts(changed){
  try{ prototypeProductsWrite(products); }catch(e){}
  if(!changed || !changed.length) return;
  if(typeof CV_API === 'undefined') return;
  try{
    if(!(await CV_API.available()) || !CV_API.token(true)) return;
    for(const p of changed){
      const dbId = p._dbId;
      if(dbId) await CV_API.request('/products/' + dbId, {method:'PUT', admin:true, body:cvDiscountBuildProductPayload(p)});
      else await CV_API.request('/products', {method:'POST', admin:true, body:cvDiscountBuildProductPayload(p)});
    }
  }catch(e){
    console.warn('Discount saved locally, backend product update failed:', e);
    throw e;
  }
}
async function applyDiscount(){
  ...
  if(!percent || percent < 0 || percent > 90){ showAdminStatus('Discount must be between 1 and 90.', true); return; }
  const changed = [];
  if(type === 'all'){
    products = products.map(function(p){ const np = {...p, discountPercent: active ? percent : 0}; changed.push(np); return np; });
  }else if(type === 'category'){
    const category = document.getElementById('discountCategoryTarget')?.value || '';
    if(!category){ showAdminStatus('Choose a category.', true); return; }
    products = products.map(function(p){
      if(String(p.category || p.category_name || '') === String(category)){ const np = {...p, discountPercent: active ? percent : 0}; changed.push(np); return np; }
      return p;
    });
  }else{
    const productId = document.getElementById('discountProductTarget')?.value || '';
    ...
    if(!productId){ showAdminStatus('Choose a product first.', true); return; }
    if((scope === 'size' || scope === 'size_fabric') && !size){ showAdminStatus('Choose a size for this product.', true); return; }
    if((scope === 'fabric' || scope === 'size_fabric') && !fabric){ showAdminStatus('Choose a fabric for this product.', true); return; }
    products = products.map(function(p){
      if(cvDiscountProductId(p) !== String(productId)) return p;
      const np = {...p};
      if(scope === 'product'){
        np.discountPercent = active ? percent : 0;
      }else{
        const rules = Array.isArray(np.discountRules) ? np.discountRules.slice() : [];
        const ruleKey = [scope, size || '', fabric || ''].join('|');
        const nextRule = {scope, size:size || null, fabric:fabric || null, percent, active, key:ruleKey};
        const idx = rules.findIndex(function(r){ return [r.scope, r.size || '', r.fabric || ''].join('|') === ruleKey; });
        if(idx >= 0) rules[idx] = nextRule; else rules.push(nextRule);
        np.discountRules = rules;
      }
      changed.push(np);
      return np;
    });
  ...
  try{
    await cvDiscountSaveChangedProducts(changed);
    renderProductsAdmin();
    renderDiscountTargets();
    showAdminStatus('Discount applied and saved.');
  }catch(e){
    renderProductsAdmin(); renderDiscountList();
    showAdminStatus('Discount applied locally, but backend save failed. Login again and press Save Discount Page.', true);
  }
}
async function clearDiscounts(){
  ...
  await cvDiscountReloadProducts();
  const changed = [];
  products = products.map(function(p){ const np = {...p, discountPercent:0, discountRules:[]}; changed.push(np); return np; });
  try{ await cvDiscountSaveChangedProducts(changed); showAdminStatus('All discounts cleared.'); }catch(e){ showAdminStatus('Discounts cleared locally, backend save failed.', true); }
  renderProductsAdmin(); renderDiscountList();
}
function renderDiscountList(){
  const el = document.getElementById('discountList');
  ...
  (products || []).forEach(function(p){
    if(Number(p.discountPercent || 0) > 0){
      rows.push('<div class="admin-item"><div><strong>'+cvDiscountEsc(cvDiscountProductLabel(p))+'</strong><br>Product discount: '+Number(p.discountPercent || 0)+'%</div><button type="button" onclick="editProduct(\''+cvDiscountEsc(cvDiscountProductId(p))+'\')">Edit</button></div>');
    }
    (Array.isArray(p.discountRules) ? p.discountRules : []).forEach(function(r){
      if(!r || !r.active) return;
      const parts = [];
      if(r.size) parts.push('Size: '+r.size);
      if(r.fabric) parts.push('Fabric: '+r.fabric);
      rows.push('<div class="admin-item"><div><strong>'+cvDiscountEsc(cvDiscountProductLabel(p))+'</strong><br>'+cvDiscountEsc(r.scope || 'rule')+' discount: '+Number(r.percent || 0)+'% '+cvDiscountEsc(parts.join(' | '))+'</div><button type="button" onclick="editProduct(\''+cvDiscountEsc(cvDiscountProductId(p))+'\')">Edit</button></div>');
    });
  });
...
async function saveDiscountPage(){
  if(typeof hasAdminPermission === 'function' && !hasAdminPermission('discounts','write')){ showAdminStatus('You have read-only access for this section.', true); return; }
  try{
    await cvDiscountSaveChangedProducts(products || []);
    localStorage.setItem('discountCodes', JSON.stringify(getDiscountCodes()));
    showAdminStatus('Discount page saved successfully.');
  }catch(e){
...
document.addEventListener('DOMContentLoaded', function(){
  setTimeout(function(){
    document.querySelectorAll('.cms-tabs button[data-tab="discountControl"]').forEach(function(btn){
      btn.addEventListener('click', function(){ setTimeout(function(){ renderDiscountTargets(); }, 80); });
    });
    if(document.getElementById('discountControl')?.classList.contains('active')) renderDiscountTargets();
  }, 300);
});
