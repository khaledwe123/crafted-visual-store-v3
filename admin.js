...
function renderDiscountTargets(){
  ...
  if(categorySelect){
    ...
    categorySelect.innerHTML = cats.length ? cats.map(function(cat){ return `<option value="${cvDiscountEscape(cat)}">${cvDiscountEscape(cat)}</option>`; }).join("") : `<option value="">No categories found</option>`;
    if(previousCategory && [...categorySelect.options].some(function(o){ return o.value === previousCategory; })) categorySelect.value = previousCategory;
    categorySelect.style.display = targetType === "category" ? "" : "none";
  }

  let productOptions = products.slice();
  if(targetType === "category") productOptions = productOptions.filter(function(p){ return String(p.category || "") === String(categorySelect?.value || ""); });
  productSelect.innerHTML = productOptions.length ? productOptions.map(function(p){ return `<option value="${cvDiscountEscape(p.id)}">${cvDiscountEscape((p.name || p.id) + (p.category ? ' — ' + p.category : ''))}</option>`; }).join("") : `<option value="">No products found</option>`;
  if(previousProduct && [...productSelect.options].some(function(o){ return o.value === previousProduct; })) productSelect.value = previousProduct;
  ...
  const p = cvDiscountProductById(productSelect.value);
  ...
  if(sizeSelect){
    ...
    sizeSelect.innerHTML = sizes.length ? sizes.map(function(size){ return `<option value="${cvDiscountEscape(size)}">${cvDiscountEscape(size)}</option>`; }).join("") : `<option value="">No sizes found for selected product</option>`;
  }
  if(fabricSelect){
    fabricSelect.style.display = showFabric ? "" : "none";
    fabricSelect.innerHTML = fabrics.length ? fabrics.map(function(fabric){ return `<option value="${cvDiscountEscape(fabric)}">${cvDiscountEscape(fabric)}</option>`; }).join("") : `<option value="">No fabrics found for selected product</option>`;
  }
