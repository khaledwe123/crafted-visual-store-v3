...
function cvDiscountEscape(value){
  return String(value == null ? "" : value).replace(/[&<>'"]/g, function(ch){
    return ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[ch];
  });
}
function cvDiscountProductById(id){
  return products.find(function(p){ return String(p.id) === String(id); }) || products[0] || null;
}
function cvDiscountProductSizes(p){
  if(!p) return [];
  if(Array.isArray(p.sizeOptions) && p.sizeOptions.length) return p.sizeOptions.map(function(s){ return s.label || String(s); }).filter(Boolean);
  if(Array.isArray(p.sizes) && p.sizes.length) return p.sizes.map(String).filter(Boolean);
  return [];
}
function cvDiscountProductFabrics(p){
  if(!p) return [];
  if(Array.isArray(p.fabricOptions) && p.fabricOptions.length) return p.fabricOptions.map(function(f){ return f.label || String(f); }).filter(Boolean);
  if(Array.isArray(p.fabrics) && p.fabrics.length) return p.fabrics.map(String).filter(Boolean);
  return [];
}
function cvEnsureDiscountRules(p){
  if(!p.discountRules || !Array.isArray(p.discountRules)) p.discountRules = [];
  return p.discountRules;
}
function cvDiscountCategoryName(c){
  if(!c) return "";
  if(typeof c === "string") return c;
  return c.label_en || c.name || c.category || "";
}
function cvDiscountVisibleCategories(){
  const fromCategories = (categories || []).filter(function(c){ return c && c.visible !== false; }).map(cvDiscountCategoryName).filter(Boolean);
  const fromProducts = products.map(function(p){ return p.category || ""; }).filter(Boolean);
  return Array.from(new Set(fromCategories.concat(fromProducts)));
}
function cvDiscountProductsForTarget(targetType, categoryValue, productId){
  if(targetType === "all") return products.slice();
  if(targetType === "category") return products.filter(function(p){ return String(p.category || "") === String(categoryValue || ""); });
  return products.filter(function(p){ return String(p.id) === String(productId || ""); });
}
function cvDiscountProductLabel(p){
  if(!p) return "";
  const name = p.name || p.name_en || p.title || p.sku || p.id || "Product";
  const category = p.category || p.category_name || "";
  return name + (category ? " — " + category : "");
}
function cvDiscountSetSelectOptions(select, items, selectedValue, emptyText){
  if(!select) return;
  if(!items || !items.length){
    select.innerHTML = `<option value="">${cvDiscountEscape(emptyText || "No options found")}</option>`;
    select.value = "";
    return;
  }
  select.innerHTML = items.join("");
  if(selectedValue && [...select.options].some(function(o){ return o.value === selectedValue; })){
    select.value = selectedValue;
  }else{
    select.selectedIndex = 0;
  }
}
let cvDiscountReloadingProducts = false;
async function cvDiscountReloadProductsIfEmpty(){
  if(cvDiscountReloadingProducts || (Array.isArray(products) && products.length)) return;
  cvDiscountReloadingProducts = true;
  try{
    const loaded = await loadProductsDataAdmin();
    ...
    console.warn("Could not reload products for discount selector", e);
  }finally{
    cvDiscountReloadingProducts = false;
  }
}
function renderDiscountTargets(){
  const targetTypeEl = document.getElementById("discountTargetType");
  const categorySelect = document.getElementById("discountCategoryTarget");
  const productSelect = document.getElementById("discountProductTarget") || document.getElementById("discountTarget");
  const scopeEl = document.getElementById("discountScope");
  const sizeSelect = document.getElementById("discountSizeTarget");
  const fabricSelect = document.getElementById("discountFabricTarget");
  const optionBox = document.getElementById("discountOptionSelectors");
  if(!productSelect) return;

  if(!Array.isArray(products) || !products.length){
    productSelect.style.display = "";
    productSelect.innerHTML = `<option value="">Loading products...</option>`;
    cvDiscountReloadProductsIfEmpty();
  }

  const targetType = targetTypeEl?.value || "product";
  const previousCategory = categorySelect?.value || "";
  const previousProduct = productSelect.value || "";

  if(categorySelect){
    const cats = cvDiscountVisibleCategories();
    const categoryItems = cats.map(function(cat){ return `<option value="${cvDiscountEscape(cat)}">${cvDiscountEscape(cat)}</option>`; });
    cvDiscountSetSelectOptions(categorySelect, categoryItems, previousCategory, "No categories found");
    categorySelect.style.display = targetType === "category" ? "" : "none";
  }

  let productOptions = Array.isArray(products) ? products.slice() : [];
  if(targetType === "category"){
    productOptions = productOptions.filter(function(p){ return String(p.category || "") === String(categorySelect?.value || ""); });
  }
  const productItems = productOptions.map(function(p){
    return `<option value="${cvDiscountEscape(p.id)}">${cvDiscountEscape(cvDiscountProductLabel(p))}</option>`;
  });
  cvDiscountSetSelectOptions(productSelect, productItems, previousProduct, "No products found");
  productSelect.style.display = targetType === "product" ? "" : "none";

  if(scopeEl){
    scopeEl.disabled = targetType !== "product";
    if(targetType !== "product") scopeEl.value = "product";
    [...scopeEl.options].forEach(function(opt){ opt.disabled = targetType !== "product" && opt.value !== "product"; });
  }

  const scope = scopeEl?.value || "product";
  const p = targetType === "product" ? cvDiscountProductById(productSelect.value) : null;
  const sizes = cvDiscountProductSizes(p);
  const fabrics = cvDiscountProductFabrics(p);
  const showSize = targetType === "product" && (scope === "size" || scope === "size_fabric");
  const showFabric = targetType === "product" && (scope === "fabric" || scope === "size_fabric");

  if(sizeSelect){
    sizeSelect.style.display = showSize ? "" : "none";
    const sizeItems = sizes.map(function(size){ return `<option value="${cvDiscountEscape(size)}">${cvDiscountEscape(size)}</option>`; });
    cvDiscountSetSelectOptions(sizeSelect, sizeItems, sizeSelect.value, "No sizes found for selected product");
  }
  if(fabricSelect){
    fabricSelect.style.display = showFabric ? "" : "none";
    const fabricItems = fabrics.map(function(fabric){ return `<option value="${cvDiscountEscape(fabric)}">${cvDiscountEscape(fabric)}</option>`; });
    cvDiscountSetSelectOptions(fabricSelect, fabricItems, fabricSelect.value, "No fabrics found for selected product");
  }
  if(optionBox) optionBox.style.display = (showSize || showFabric) ? "" : "none";
}
function applyDiscount(){
  ...
  const targetType = document.getElementById("discountTargetType")?.value || "product";
  const categoryValue = document.getElementById("discountCategoryTarget")?.value || "";
  const productId = (document.getElementById("discountProductTarget") || document.getElementById("discountTarget"))?.value || "";
  ...
  const active = (document.getElementById("discountStatusTarget")?.value || "active") === "active";
  const editingId = document.getElementById("editingDiscountRuleId")?.value || "";

  if(percent < 0 || percent > 90){ alert("Discount must be between 0 and 90"); return; }
  if(targetType === "category" && !categoryValue){ showAdminStatus("Choose a product category first.", true); return; }
  if(targetType === "product" && !productId){ showAdminStatus("Choose a specific product first.", true); return; }
  if(targetType !== "product" && scope !== "product"){ showAdminStatus("Size and fabric discounts are available only when Specific Product is selected.", true); return; }
  if((scope === "size" || scope === "size_fabric") && !size){ showAdminStatus("Choose a size for this product discount.", true); return; }
  if((scope === "fabric" || scope === "size_fabric") && !fabric){ showAdminStatus("Choose a fabric for this product discount.", true); return; }

  const targetProducts = cvDiscountProductsForTarget(targetType, categoryValue, productId);
  if(!targetProducts.length){ showAdminStatus("No products match this discount target.", true); return; }
  const targetIds = new Set(targetProducts.map(function(p){ return String(p.id); }));
  const sharedBatchId = editingId || ("dr_" + Date.now());

  products = products.map(function(p){
    if(!targetIds.has(String(p.id))) return p;
    const rules = cvEnsureDiscountRules(p);
    const ruleId = targetType === "product" ? sharedBatchId : (sharedBatchId + "_" + String(p.id));
    const rule = {
      id: ruleId,
      batchId: sharedBatchId,
      targetType: targetType,
      category: targetType === "category" ? categoryValue : "",
      scope: scope,
      percent: percent,
      size: targetType === "product" ? size : "",
      fabric: targetType === "product" ? fabric : "",
      active: active,
      updatedAt: new Date().toISOString()
    };
    const existingIndex = rules.findIndex(function(r){ return String(r.id) === String(rule.id) || (editingId && String(r.batchId || '') === String(editingId)); });
    if(existingIndex >= 0) rules[existingIndex] = Object.assign({}, rules[existingIndex], rule);
    else rules.push(rule);
    if(scope === "product") p.discountPercent = active ? percent : 0;
    return Object.assign({}, p, {discountRules:rules});
  });
  const editEl = document.getElementById("editingDiscountRuleId");
  if(editEl) editEl.value = "";
  prototypeProductsWrite(products);
  renderProductsAdmin(); renderDiscountList(); renderDiscountTargets();
  const msg = targetType === "all" ? "Discount applied to all products." : targetType === "category" ? "Discount applied to selected category products." : "Discount applied to selected product.";
  showAdminStatus(msg + " Press Save Discount Page to publish permanently.");
}
function clearDiscounts(){
  if(typeof hasAdminPermission === "function" && !hasAdminPermission("discounts","write")){ showAdminStatus("You have read-only access for this section.", true); return; }
  if(!confirm("Clear all product, size, fabric, and size + fabric discounts?")) return;
  products = products.map(function(p){ return Object.assign({}, p, {discountPercent:0, discountRules:[]}); });
  prototypeProductsWrite(products);
  renderProductsAdmin(); renderDiscountList();
  showAdminStatus("All discount rules cleared. Press Save Discount Page to publish permanently.");
}
function toggleDiscountRule(productId, ruleId){
  if(typeof hasAdminPermission === "function" && !hasAdminPermission("discounts","write")){ showAdminStatus("You have read-only access for this section.", true); return; }
  products = products.map(function(p){
    if(String(p.id) !== String(productId)) return p;
    const rules = cvEnsureDiscountRules(p).map(function(r){
      if(String(r.id) === String(ruleId)) return Object.assign({}, r, {active:!r.active, updatedAt:new Date().toISOString()});
      return r;
    });
    const productRule = rules.find(function(r){ return r.scope === "product" && String(r.id) === String(ruleId); });
    if(productRule) p.discountPercent = productRule.active ? Number(productRule.percent || 0) : 0;
    return Object.assign({}, p, {discountRules:rules});
  });
  prototypeProductsWrite(products); renderProductsAdmin(); renderDiscountList();
}
function deleteDiscountRule(productId, ruleId){
  if(typeof hasAdminPermission === "function" && !hasAdminPermission("discounts","write")){ showAdminStatus("You have read-only access for this section.", true); return; }
  products = products.map(function(p){
    if(String(p.id) !== String(productId)) return p;
    const rule = cvEnsureDiscountRules(p).find(function(r){ return String(r.id) === String(ruleId); });
    const rules = cvEnsureDiscountRules(p).filter(function(r){ return String(r.id) !== String(ruleId); });
    if(rule && rule.scope === "product") p.discountPercent = 0;
    return Object.assign({}, p, {discountRules:rules});
  });
  prototypeProductsWrite(products); renderProductsAdmin(); renderDiscountList();
}
function editDiscountRule(productId, ruleId){
  const p = cvDiscountProductById(productId);
  const rule = p && cvEnsureDiscountRules(p).find(function(r){ return String(r.id) === String(ruleId); });
  if(!p || !rule) return;
  const targetTypeEl = document.getElementById("discountTargetType");
  const categoryEl = document.getElementById("discountCategoryTarget");
  const scopeEl = document.getElementById("discountScope");
  const productEl = document.getElementById("discountProductTarget") || document.getElementById("discountTarget");
  const percentEl = document.getElementById("bulkDiscount");
  const editEl = document.getElementById("editingDiscountRuleId");
  const targetType = rule.targetType || "product";
  if(targetTypeEl) targetTypeEl.value = targetType;
  renderDiscountTargets();
  if(categoryEl && targetType === "category") categoryEl.value = rule.category || p.category || "";
  if(productEl) productEl.value = p.id;
  if(scopeEl) scopeEl.value = rule.scope || "product";
  renderDiscountTargets();
  if(document.getElementById("discountSizeTarget")) document.getElementById("discountSizeTarget").value = rule.size || "";
  if(document.getElementById("discountFabricTarget")) document.getElementById("discountFabricTarget").value = rule.fabric || "";
  if(document.getElementById("discountStatusTarget")) document.getElementById("discountStatusTarget").value = rule.active === false ? "inactive" : "active";
  if(percentEl) percentEl.value = Number(rule.percent || 0);
  if(editEl) editEl.value = rule.batchId || rule.id;
  showAdminStatus("Editing discount rule. Change the fields, then press Apply Discount.");
}
function renderDiscountList(){
  ...
  products.forEach(function(p){
    const rules = cvEnsureDiscountRules(p);
    if(!rules.length && Number(p.discountPercent || 0) > 0){
      const rule = {id:"legacy_"+p.id, scope:"product", percent:Number(p.discountPercent||0), active:true};
      rules.push(rule);
      p.discountRules = rules;
    }
    rules.forEach(function(r){
      rows.push({product:p, rule:r});
    });
  });
  el.innerHTML = rows.length ? rows.map(function(item){
    const p = item.product, r = item.rule;
    const scopeLabel = r.scope === "size_fabric" ? "Size + Fabric" : (r.scope || "product").replace(/^./, function(x){return x.toUpperCase();});
    const targetLabel = r.targetType === "all" ? "All Products" : (r.targetType === "category" ? "Category: " + (r.category || p.category || "") : "Specific Product");
    const target = [targetLabel, r.size ? "Size: "+r.size : "", r.fabric ? "Fabric: "+r.fabric : ""].filter(Boolean).join(" | ") || "Whole product";
    const status = r.active === false ? "Inactive" : "Active";
    return `<div class="admin-item"><div><strong>${cvDiscountEscape(p.name || '')}</strong><br>${cvDiscountEscape(scopeLabel)} | ${cvDiscountEscape(target)}<br>${Number(r.percent||0)}% discount | Status: ${status}</div><div><button type="button" onclick="editDiscountRule('${cvDiscountEscape(p.id)}','${cvDiscountEscape(r.id)}')">Edit</button><button type="button" onclick="toggleDiscountRule('${cvDiscountEscape(p.id)}','${cvDiscountEscape(r.id)}')">${r.active === false ? "Enable" : "Disable"}</button><button type="button" onclick="deleteDiscountRule('${cvDiscountEscape(p.id)}','${cvDiscountEscape(r.id)}')">Delete</button></div></div>`;
  }).join("") : "<p>No active discounts.</p>";
}
