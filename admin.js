...
function renderDiscountTargets(){
  ...
  const statusSelect = document.getElementById("discountStatusTarget");
  if(!productSelect) return;
  const previousProduct = productSelect.value;
  productSelect.innerHTML = products.map(function(p){ return `<option value="${cvDiscountEscape(p.id)}">${cvDiscountEscape(p.name || p.id)}</option>`; }).join("");
  ...
  if(sizeSelect){
    sizeSelect.style.display = (scope === "size" || scope === "size_fabric") ? "" : "none";
  ...
  if(fabricSelect){
    fabricSelect.style.display = (scope === "fabric" || scope === "size_fabric") ? "" : "none";
...
function applyDiscount(){
  ...
  if((scope === "size" || scope === "size_fabric") && !size){ showAdminStatus("Choose a size for this discount.", true); return; }
  if((scope === "fabric" || scope === "size_fabric") && !fabric){ showAdminStatus("Choose a fabric for this discount.", true); return; }
  products = products.map(function(p){
    if(String(p.id) !== String(productId)) return p;
    const rules = cvEnsureDiscountRules(p);
    const rule = { id: editingId || ("dr_" + Date.now()), scope:scope, percent:percent, size:size, fabric:fabric, active:active, updatedAt:new Date().toISOString() };
    const existingIndex = rules.findIndex(function(r){ return String(r.id) === String(rule.id); });
  ...
  showAdminStatus("Discount rule applied. Press Save Discount Page to publish permanently.");
}
...
function renderDiscountList(){
  ...
  el.innerHTML = rows.length ? rows.map(function(item){
    ...
    const target = [r.size ? "Size: "+r.size : "", r.fabric ? "Fabric: "+r.fabric : ""].filter(Boolean).join(" | ") || "Whole product";
